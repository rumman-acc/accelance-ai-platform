...
**TODO (2026-08-19):** the summary counts above don't yet reflect the Guardrails v2
rearchitecture rows changed the same day (Guardrails catalog ✅-leaning→🟡 40%, Policy templates
✅→🔴) — `rules/epics-feature-status.md` §9/§10 is up to date and remains the source of truth;
re-derive this file's summary line from it rather than trusting the number above until it's
recounted.
...
| Feature | What it's for | Status | Built | Scope / what's left |
| --- | --- | --- | --- | --- |
| Guardrails catalog | **Superseded 2026-08-19** by a DB-driven Kind/Definition/Node-instance model per `Guardrails_build_plan.md` — see `rules/guardrails-v2/`. `/guardrails` is now a plain read-only catalog browser (13 definitions across 5 categories); no workspace-wide toggles, override counts, or custom-catalog authoring on that page anymore. | 🟡 | 40% | Phase 0 (kind list, verdict contract, definition schema, code audit, full 14-row reconciliation — all written) + Phase 1 (3 new entities, 5-migration batch seeding 13 definitions + backfilling 147 `GuardrailFlowAttachment` rows for the 7 chatflow-scoped keys) are done and verified against the real dev DB, including a re-run idempotency check on the backfill. The 7 backfilled keys' actual block/allow decision is still made by the OLD `GuardrailPolicy` path unchanged (new path only records a shadow verdict) — real per-agent overrides via `GuardrailsCompliance.jsx` and the `/compliance` page's `data_retention_policy` toggle both still work, verified live. One fix required special handling: `known-issues.md` #016 (a `databaseEntities` bug that had silently disabled Prompt-Injection Defense/Egress Filtering) makes their old path functional for the first time, which would itself have been a production behavior change for an already-toggled-on workspace — gated behind an explicit, currently-never-set promotion flag, verified directly against real DB state that the gate holds. mysql/mariadb/sqlite migrations are written but not exercised against real instances — residual risk, not verified. Phase 2+ (real canvas anchor, inline ports, config panel, authoring, dry-run tester) not started. |
