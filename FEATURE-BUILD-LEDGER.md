...
| Feature | What it's for | Status | Built | Scope / what's left |
| --- | --- | --- | --- | --- |
| Guardrails catalog | **Superseded 2026-08-19** by a DB-driven Kind/Definition/Node-instance model per `Guardrails_build_plan.md` — see `rules/guardrails-v2/`. `/guardrails` is now a plain read-only catalog browser (13 definitions across 5 categories); no workspace-wide toggles, override counts, or custom-catalog authoring on that page anymore. | 🟡 | 60% | Phase 0 + Phase 1 (as before) done and verified, including the `UNIQUE(key, version)` constraint added 2026-08-19. **Phase 2 (2026-08-20):** a real `guardrails` canvas anchor now exists — but on classic `ToolAgent.ts`/`AgentAsTool.ts`, not AgentFlow V2 as originally planned (AgentFlow V2 has no anchor mechanism at all, discovered mid-build). 3 real physical node files (`egress_filtering`, `prompt_injection_defense`, `confused_deputy_prevention`) ship instead of the originally-planned DB-synthesized ones (impossible on the classic build path — a node must be a real file to be `.init()`-able). **Live-verified 2026-08-20** end to end on the real dev instance/DB, including a genuine LLM-backed tool call: palette entry, drag/connect/save/reload persistence, the exact resolved-config shape at runtime, a shadow-mode run (verdict recorded, call still succeeded), a promote-to-block run scoped to one node instance (call actually blocked), and reverting back to shadow. Full evidence in `rules/guardrails-v2/phase2-canvas.md`. Not separately covered: the same proof for `AgentAsTool.ts`/Confused Deputy Prevention, and an existing-flow-regression check — both low residual risk. Dynamic DB-driven node registration (needed for a future custom-authoring phase) remains entirely unbuilt. Inline ports, a general config-panel authoring flow, and the dry-run tester also not started. |
