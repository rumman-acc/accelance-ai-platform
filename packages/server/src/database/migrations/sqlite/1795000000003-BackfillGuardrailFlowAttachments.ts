 ...
 *
 * SQLite has no native UUID() function; the generated id below is the standard SQLite UUID v4
 * idiom (random hex blocks assembled into RFC4122-shaped text) -- fine for a one-time data
 * migration on the driver used for local/dev/test, not the production Postgres deployment.
 */
export class BackfillGuardrailFlowAttachments1795000000003 implements MigrationInterface {
    ...
    private readonly uuidExpr = `(lower(hex(randomblob(4))) || '-' || lower(hex(randomblob(2))) || '-4' || substr(lower(hex(randomblob(2))),2) || '-' || substr('89ab', abs(random()) % 4 + 1, 1) || substr(lower(hex(randomblob(2))),2) || '-' || lower(hex(randomblob(6))))`
        ...
        for (const key of this.keys) {
            await queryRunner.query(
                `INSERT INTO "guardrail_flow_attachment"
                 ...
                 SELECT ${this.uuidExpr}, gp."workspaceId", gp."chatflowId", gd."id", gd."key", gd."kindKey",
            ...
            await queryRunner.query(
                `INSERT INTO "guardrail_flow_attachment"
                    ("id","workspaceId","chatflowId","definitionId","definitionKey","kindKey","paramsSnapshot","onFailAction","failMode","timeoutMs","observeMode")
                 SELECT ${this.uuidExpr}, gp."workspaceId", cf."id", gd."id", gd."key", gd."kindKey",
                        COALESCE(gp."config", gd."defaultParams"), gd."defaultOnFailAction", gd."defaultFailMode", gd."defaultTimeoutMs", 1
                 FROM "guardrail_policy" gp
                 JOIN "guardrail_definition" gd ON gd."key" = gp."catalogKey" AND gd."workspaceId" IS NULL
