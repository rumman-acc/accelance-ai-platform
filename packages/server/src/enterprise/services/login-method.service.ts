...
export class LoginMethodService {
    ...
    public validateLoginMethodId(id: string | undefined) {
        if (isInvalidUUID(id)) throw new InternalAccelanceError(StatusCodes.BAD_REQUEST, LoginMethodErrorMessage.INVALID_LOGIN_METHOD_ID)
    }
    ...
    public validateLoginMethodName(name: string | undefined) {
        if (isInvalidName(name))
            throw new InternalAccelanceError(StatusCodes.BAD_REQUEST, LoginMethodErrorMessage.INVALID_LOGIN_METHOD_NAME)
    }
    ...
    public validateLoginMethodStatus(status: string | undefined) {
        if (status && !Object.values(LoginMethodStatus).includes(status as LoginMethodStatus))
            throw new InternalAccelanceError(StatusCodes.BAD_REQUEST, LoginMethodErrorMessage.INVALID_LOGIN_METHOD_STATUS)
    }
    ...
    public async encryptLoginMethodConfig(config: string | undefined) {
        if (!config) throw new InternalAccelanceError(StatusCodes.BAD_REQUEST, LoginMethodErrorMessage.INVALID_LOGIN_METHOD_STATUS)
    ...
    public async createLoginMethod(data: Partial<LoginMethod>) {
        ...
        try {
            ...
            if (!createdBy) throw new InternalAccelanceError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
    ...
    public async createOrUpdateConfig(body: any) {
        ...
        try {
            ...
            if (!createdOrUpdatedByUser) throw new InternalAccelanceError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
            const organization = await this.organizationService.readOrganizationById(organizationId, queryRunner)
            if (!organization) throw new InternalAccelanceError(StatusCodes.NOT_FOUND, OrganizationErrorMessage.ORGANIZATION_NOT_FOUND)
    ...
    public async updateLoginMethod(newLoginMethod: Partial<LoginMethod>) {
        ...
        if (!oldLoginMethod) throw new InternalAccelanceError(StatusCodes.NOT_FOUND, LoginMethodErrorMessage.LOGIN_METHOD_NOT_FOUND)
        const updatedBy = await this.userService.readUserById(newLoginMethod.updatedBy, queryRunner)
        if (!updatedBy) throw new InternalAccelanceError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
