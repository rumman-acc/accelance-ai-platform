...
export class WorkspaceService {
    ...
    public validateWorkspaceId(id: string | undefined) {
        if (isInvalidUUID(id)) throw new InternalAccelanceError(StatusCodes.BAD_REQUEST, WorkspaceErrorMessage.INVALID_WORKSPACE_ID)
    }
    ...
    public validateWorkspaceName(name: string | undefined, isRegister: boolean = false) {
        if (isInvalidName(name)) throw new InternalAccelanceError(StatusCodes.BAD_REQUEST, WorkspaceErrorMessage.INVALID_WORKSPACE_NAME)
        if (!isRegister && (name === WorkspaceName.DEFAULT_PERSONAL_WORKSPACE || name === WorkspaceName.DEFAULT_WORKSPACE)) {
            throw new InternalAccelanceError(StatusCodes.BAD_REQUEST, WorkspaceErrorMessage.WORKSPACE_RESERVERD_NAME)
        }
    }
    ...
    public async readWorkspaceByOrganizationId(organizationId: string | undefined, queryRunner: QueryRunner) {
        ...
        if (!rolePersonalWorkspace) throw new InternalAccelanceError(StatusCodes.NOT_FOUND, RoleErrorMessage.ROLE_NOT_FOUND)
    ...
    public async createWorkspace(data: Partial<Workspace>) {
        ...
        // Policy Templates (the retroactive-apply-a-default-bundle mechanism) is removed per
        // Guardrails v2 §2.2 -- new workspaces no longer get an auto-applied guardrail bundle.
        // See rules/guardrails-v2/ and the implementation plan.
    ...
    public async updateWorkspace(newWorkspaceData: Partial<Workspace>) {
        ...
        if (!oldWorkspaceData) throw new InternalAccelanceError(StatusCodes.NOT_FOUND, WorkspaceErrorMessage.WORKSPACE_NOT_FOUND)
