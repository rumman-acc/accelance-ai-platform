...
const getAllPermissions = async (req, res, next) => {
    try {
        ...
        if (type !== 'ROLE' && appServer.identityManager.getPlatformType() === Interface_1.Platform.CLOUD) {
        ...
        if (type !== 'ROLE' && user.isOrganizationAdmin === false) {
            ...
            const filteredPermissions = {};
            for (const [category, categoryPermissions] of Object.entries(permissions)) {
                const filteredCategoryPermissions = categoryPermissions.filter((permission) => userPermissions?.includes(permission.key));
                if (filteredCategoryPermissions.length > 0) {
                    filteredPermissions[category] = filteredCategoryPermissions;
                }
            }
        ...
        return res.status(http_status_codes_1.StatusCodes.OK).json(permissions);
    }
    catch (error) {
        next(error);
    }
