...
exports.getSecureAppUrl = getSecureAppUrl;
exports.getSecureTokenLink = getSecureTokenLink;
...
function getSecureAppUrl(path) {
    ...
    const isLocalhost = urlObj.hostname === 'localhost' || urlObj.hostname === '127.0.0.1' || urlObj.hostname === '[::1]' || urlObj.hostname === '0.0.0.0';
    ...
    if (urlObj.protocol === 'http:' && !isLocalhost) {
        ...
        logger_1.default.warn(`APP_URL uses insecure HTTP protocol for non-localhost URL. ` +
 ...
 */
function getSecureTokenLink(path, token) {
