...
describe('createWebhook', () => {
    beforeEach(() => {
        ...
        mockValidateWebhookChatflow.mockResolvedValue({ responseMode: 'sync' });
    });
    it('calls next with PRECONDITION_FAILED when id is missing', async () => {
        const req = mockReq({ params: {} });
        const res = mockRes();
        const next = mockNext();
        await index_1.default.createWebhook(req, res, next);
        expect(next).toHaveBeenCalledWith(expect.objectContaining({ statusCode: http_status_codes_1.StatusCodes.PRECONDITION_FAILED }));
    });
    it('calls next with error when validateWebhookChatflow rejects', async () => {
        const error = { statusCode: http_status_codes_1.StatusCodes.NOT_FOUND, message: 'not found' };
    ...
    it('wraps req.body under webhook key before calling buildChatflow', async () => {
        ...
        const next = mockNext();
        await index_1.default.createWebhook(req, res, next);
        expect(mockBuildChatflow).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('redacts credential-bearing headers before they reach the flow', async () => {
        ...
        const next = mockNext();
        await index_1.default.createWebhook(req, res, next);
        const passedHeaders = mockBuildChatflow.mock.calls[0][0].body.webhook.headers;
    ...
    it('passes the original body to validateWebhookChatflow before mutation', async () => {
        ...
        const next = mockNext();
        await index_1.default.createWebhook(req, res, next);
        expect(mockValidateWebhookChatflow).toHaveBeenCalledWith('chatflow-123', undefined, { foo: 'bar' }, 'POST', expect.any(Object), expect.any(Object), undefined, // rawBody — not set on mock request
    ...
    it('passes skipFieldValidation option when body contains humanInput (resume call)', async () => {
        ...
        const next = mockNext();
        await index_1.default.createWebhook(req, res, next);
        expect(mockValidateWebhookChatflow).toHaveBeenCalledWith('chatflow-123', undefined, expect.objectContaining({ humanInput: expect.any(Object) }), 'POST', expect.any(Object), expect.any(Object), undefined, { skipFieldValidation: true });
    });
    ...
    it('returns 202 immediately when callbackUrl is configured on Start node', async () => {
        mockValidateWebhookChatflow.mockResolvedValue({ responseMode: 'async', callbackUrl: 'https://cb.example.com' });
    ...
    it('dispatches SUCCESS callback when flow completes without action', async () => {
        ...
        const req = mockReq();
        const res = mockRes();
        await index_1.default.createWebhook(req, res, mockNext());
        expect(mockDispatchCallback).toHaveBeenCalledWith('https://cb.example.com', { status: 'SUCCESS', chatId: expect.any(String), data: apiResponse }, undefined);
    });
    it('dispatches STOPPED callback when flow has action (HITL pause)', async () => {
        ...
        const res = mockRes();
        await index_1.default.createWebhook(req, res, mockNext());
        expect(mockDispatchCallback).toHaveBeenCalledWith('https://cb.example.com', {
            status: 'STOPPED',
            chatId: expect.any(String),
    ...
    it('dispatches ERROR callback when flow throws', async () => {
        ...
        const res = mockRes();
        await index_1.default.createWebhook(req, res, mockNext());
        expect(mockDispatchCallback).toHaveBeenCalledWith('https://cb.example.com', { status: 'ERROR', chatId: expect.any(String), error: 'flow exploded' }, undefined);
    });
    ...
    it('calls next with BAD_REQUEST when node callbackUrl is not a valid http/https URL', async () => {
        mockValidateWebhookChatflow.mockResolvedValue({ responseMode: 'async', callbackUrl: 'ftp://bad.example.com' });
        const req = mockReq();
        const res = mockRes();
        const next = mockNext();
        await index_1.default.createWebhook(req, res, next);
    ...
    it('returns 202 immediately in fire-and-forget mode (async on, no callback URL)', async () => {
        mockValidateWebhookChatflow.mockResolvedValue({ responseMode: 'async' });
        mockBuildChatflow.mockResolvedValue({ text: 'done' });
        const req = mockReq();
        const res = mockRes();
        const next = mockNext();
    ...
    it('opens an SSE stream when responseMode is stream and chatflow is streamable', async () => {
        mockValidateWebhookChatflow.mockResolvedValue({ responseMode: 'stream' });
    ...
    it('sets streaming=true on req.body before invoking buildChatflow in stream mode', async () => {
        ...
        const next = mockNext();
        await index_1.default.createWebhook(req, res, next);
        expect(mockBuildChatflow).toHaveBeenCalledWith(expect.objectContaining({ body: expect.objectContaining({ streaming: true }) }), Interface_1.ChatType.WEBHOOK);
    });
