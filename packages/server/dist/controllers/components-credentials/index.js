...
const getComponentByName = async (req, res, next) => {
    try {
        if (typeof req.params === 'undefined' || !req.params.name) {
            throw new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: componentsCredentialsController.getComponentByName - name not provided!`);
        }
...
const getSingleComponentsCredentialIcon = async (req, res, next) => {
    try {
        if (typeof req.params === 'undefined' || !req.params.name) {
            throw new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: componentsCredentialsController.getSingleComponentsCredentialIcon - name not provided!`);
        }
