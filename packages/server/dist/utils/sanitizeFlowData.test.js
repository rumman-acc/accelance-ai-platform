...
const sanitizeFlowData_1 = require("./sanitizeFlowData");
const makeFlowData = (nodes, edges = []) => JSON.stringify({ nodes, edges, viewport: { x: 0, y: 0, zoom: 1 } });
const makeNode = (inputs, inputParams, extra = {}) => ({
    id: 'node_0',
    position: { x: 0, y: 0 },
    type: 'customNode',
    data: {
...
describe('sanitizeFlowDataForPublicEndpoint', () => {
    it('strips password-type inputs', () => {
        ...
        const result = JSON.parse((0, sanitizeFlowData_1.sanitizeFlowDataForPublicEndpoint)(flowData));
    ...
    it('preserves startAgentflow node inputs used by the embed widget', () => {
        ...
        const flowData = makeFlowData([
            makeNode({ startInputType: 'formInput', formTitle: 'Contact Us', formDescription: 'Fill out the form', formInputTypes }, [
    ...
    it('returns empty nodes/edges structure for malformed flowData', () => {
        const result = JSON.parse((0, sanitizeFlowData_1.sanitizeFlowDataForPublicEndpoint)('not-valid-json'));
        expect(result).toEqual({ nodes: [], edges: [] });
    });
    it('returns the original string unchanged when flowDataString is empty', () => {
        expect((0, sanitizeFlowData_1.sanitizeFlowDataForPublicEndpoint)('')).toBe('');
    });
    ...
    it('does not crash when a node has no inputParams', () => {
        ...
        expect(() => (0, sanitizeFlowData_1.sanitizeFlowDataForPublicEndpoint)(flowData)).not.toThrow();
        const result = JSON.parse((0, sanitizeFlowData_1.sanitizeFlowDataForPublicEndpoint)(flowData));
        expect(result.nodes[0].data.inputs.foo).toBe('bar');
    });
});
//# sourceMappingURL=sanitizeFlowData.test.js.map