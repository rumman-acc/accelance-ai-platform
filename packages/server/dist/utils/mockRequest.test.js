...
describe('createMockRequest', () => {
    it('sets params.id to chatflowId', () => {
        const req = (0, mockRequest_1.createMockRequest)({ chatflowId: 'flow-123' });
    ...
    it('merges custom body fields with defaults', () => {
        const req = (0, mockRequest_1.createMockRequest)({
            chatflowId: 'flow-123',
    ...
    it('inherits protocol from sourceRequest', () => {
        const sourceReq = { protocol: 'https', headers: {}, get: jest.fn() };
        const req = (0, mockRequest_1.createMockRequest)({ chatflowId: 'flow-123', sourceRequest: sourceReq });
        expect(req.protocol).toBe('https');
    });
    it('delegates get() to sourceRequest when provided', () => {
        const getMock = jest.fn().mockReturnValue('example.com');
        const sourceReq = { protocol: 'https', headers: { host: 'example.com' }, get: getMock };
        const req = (0, mockRequest_1.createMockRequest)({ chatflowId: 'flow-123', sourceRequest: sourceReq });
...
//# sourceMappingURL=mockRequest.test.js.map