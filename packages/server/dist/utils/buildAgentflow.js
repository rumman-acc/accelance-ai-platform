...
exports.isObjectNotEmpty = exports.executeAgentFlow = exports.resolveVariables = void 0;
 ...
 */
const resolveWebhookRefs = (template, webhook) => {
    if (!template)
        return '';
    if (!webhook)
        return template;
    return template.replace(/{{(.*?)}}/g, (match, ref) => {
        ...
        if (/(^|\.)(__proto__|constructor|prototype)(\.|$)/.test(subPath))
            return match;
        const val = (0, lodash_1.get)(webhook, subPath);
        if (val == null)
            return match;
 ...
 */
const addExecution = async (appDataSource, agentflowId, agentFlowExecutedData, sessionId, workspaceId, userId) => {
...
const resolveVariables = async (reactFlowNodeData, question, form, flowConfig, availableVariables, variableOverrides, uploadedFilesContent, chatHistory, componentNodes, agentFlowExecutedData, iterationContext, loopCounts, webhook) => {
    ...
    const resolveNodeReference = async (value) => {
        ...
        for (const match of matches) {
            ...
            if (variableFullPath.startsWith('$webhook.')) {
                const variableValue = (0, lodash_1.get)(webhook, variableFullPath.replace('$webhook.', ''));
                if (variableValue != null) {
                    const formattedValue = Array.isArray(variableValue) || (typeof variableValue === 'object' && variableValue !== null)
                        ? JSON.stringify(variableValue)
                        : variableValue;
            ...
            if (variableFullPath === _1.CHAT_HISTORY_VAR_PREFIX) {
                resolvedValue = resolvedValue.replace(match, (0, accelance_components_1.convertChatHistoryToText)(chatHistory));
            }
 ...
 */
const executeNode = async ({ nodeId, reactFlowNode, nodes, edges, graph, reversedGraph, incomingInput, chatflow, chatId, sessionId, apiMessageId, evaluationRunId, parentExecutionId, pastChatHistory, prependedChatHistory, appDataSource, usageCacheManager, telemetry, componentNodes, cachePool, sseStreamer, baseURL, overrideConfig = {}, apiOverrideStatus = false, nodeOverrides = {}, variableOverrides = [], uploadedFilesContent = '', fileUploads, humanInput, agentFlowExecutedData = [], agentflowRuntime, abortController, parentTraceIds, analyticHandlers, isInternal, isRecursive, iterationContext, loopCounts, orgId, workspaceId, subscriptionId, productId, userId }) => {
    try {
        ...
        let formValue = {};
        if ((0, exports.isObjectNotEmpty)(incomingInput.form)) {
            formValue = incomingInput.form;
        }
        else if ((0, exports.isObjectNotEmpty)(agentflowRuntime.form)) {
            formValue = agentflowRuntime.form;
        }
        let webhookValue = {};
        if ((0, exports.isObjectNotEmpty)(incomingInput.webhook)) {
            webhookValue = incomingInput.webhook;
        }
        else if ((0, exports.isObjectNotEmpty)(agentflowRuntime.webhook)) {
            webhookValue = agentflowRuntime.webhook;
        }
        const reactFlowNodeData = await (0, exports.resolveVariables)(flowNodeData, incomingInput.question ?? '', formValue, flowConfig, availableVariables, variableOverrides, uploadedFilesContent, chatHistory, componentNodes, agentFlowExecutedData, iterationContext, loopCounts, webhookValue);
        ...
        if (incomingInput.question && (0, exports.isObjectNotEmpty)(incomingInput.form)) {
        ...
        else if ((0, exports.isObjectNotEmpty)(incomingInput.form)) {
...
const executeAgentFlow = async ({ componentNodes, incomingInput, chatflow, chatId, evaluationRunId, appDataSource, telemetry, usageCacheManager, cachePool, sseStreamer, baseURL, isInternal, uploadedFilesContent, fileUploads, signal: abortController, isRecursive = false, parentExecutionId, iterationContext, isTool = false, chatType, orgId, workspaceId, subscriptionId, productId, userId }) => {
    ...
    if (!isRecursive) {
        if (startInputType === 'webhookTrigger' && chatType !== Interface_1.ChatType.WEBHOOK) {
            const configuredMethod = (startNode?.data?.inputs?.webhookMethod || 'POST').toUpperCase();
            throw new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.BAD_REQUEST, `This flow is configured as a Webhook Trigger. Call ${configuredMethod} /api/webhook/${chatflowid} instead of the prediction API.`);
        }
        if (startInputType === 'scheduleInput' && chatType !== Interface_1.ChatType.SCHEDULED) {
            throw new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.BAD_REQUEST, `This flow is configured as a Scheduled Trigger. It is fired by the scheduler and cannot be invoked via the API. Change the Start node Input Type to Chat or Form to call it from the prediction API.`);
        }
    }
    // @ts-ignore
    if (isTool)
    ...
    */
    ...
    if (startInputType === 'webhookTrigger' && humanInput && previousExecution) {
        ...
        if (previousStartAgent) {
            ...
            if (previousStartAgentOutput && typeof previousStartAgentOutput === 'object' && 'webhook' in previousStartAgentOutput) {
                incomingInput.webhook = previousStartAgentOutput.webhook;
            }
        }
    }
    if (startInputType === 'webhookTrigger' && !humanInput) {
        const webhookInputMode = startNode?.data?.inputs?.webhookInputMode || 'text';
        if (webhookInputMode === 'text') {
            const template = startNode?.data?.inputs?.webhookDefaultInput || '';
    ...
    // per iteration.
    const loopGuardrail = await guardrails_1.default.evaluate(workspaceId, chatflowid, 'loop_recursion_detection');
    const loopGuardrailMaxSteps = loopGuardrail.enabled && typeof loopGuardrail.config?.maxSteps === 'number' ? loopGuardrail.config.maxSteps : Infinity;
    ...
    let parentTraceIds;
    try {
        if (chatflow.analytic || (0, accelance_components_1.tracingEnvEnabled)()) {
            ...
            analyticHandlers = accelance_components_1.AnalyticHandler.getInstance({ inputs: { analytics: analyticInputs } }, {
                orgId,
                workspaceId,
                appDataSource,
                databaseEntities: _1.databaseEntities,
            ...
            if (analyticHandlers?.hasActiveProviders()) {
                ...
                parentTraceIds = await analyticHandlers.onChainStart(flowName, form && Object.keys(form).length > 0 ? JSON.stringify(form) : question || '');
            }
        }
    }
    catch (error) {
    ...
    while (nodeExecutionQueue.length > 0 && status === 'INPROGRESS') {
        ...
        if (iterations > loopGuardrailMaxSteps) {
            throw new Error(`Loop & Recursion Detection: this agent exceeded its configured step budget (${loopGuardrailMaxSteps}). Raise it in the Guardrails settings if this flow legitimately needs more steps.`);
        }
    ...
    const lastNodeOutput = agentFlowExecutedData[agentFlowExecutedData.length - 1].data?.output;
    let content = lastNodeOutput?.content ?? ' ';
    ...
    try {
        ...
        logger_1.default.error('[server]: Error parsing chatflow config:', e);
    }
    if (chatflowConfig?.postProcessing?.enabled === true && content) {
        try {
            ...
            const nodeInstanceFilePath = componentNodes['customFunctionAgentflow'].filePath;
            const nodeModule = await Promise.resolve(`${nodeInstanceFilePath}`).then(s => __importStar(require(s)));
            ...
            const options = {
                ...
                postProcessing: {
                    rawOutput: content,
                    chatHistory: (0, lodash_1.cloneDeep)(chatHistory),
                    sourceDocuments: lastNodeOutput?.sourceDocuments ? (0, lodash_1.cloneDeep)(lastNodeOutput.sourceDocuments) : undefined,
                    usedTools: lastNodeOutput?.usedTools ? (0, lodash_1.cloneDeep)(lastNodeOutput.usedTools) : undefined,
                    artifacts: lastNodeOutput?.artifacts ? (0, lodash_1.cloneDeep)(lastNodeOutput.artifacts) : undefined,
                    fileAnnotations: lastNodeOutput?.fileAnnotations ? (0, lodash_1.cloneDeep)(lastNodeOutput.fileAnnotations) : undefined
                },
                appDataSource,
                databaseEntities: _1.databaseEntities,
                workspaceId,
            ...
            logger_1.default.error('[server]: Post Processing Error:', e);
        }
    }
    ...
    const userMessage = {
        ...
        chatflowid,
        chatType: chatType || (evaluationRunId ? Interface_1.ChatType.EVALUATION : isInternal ? Interface_1.ChatType.INTERNAL : Interface_1.ChatType.EXTERNAL),
        chatId,
        sessionId,
    ...
    if (lastNodeOutput?.reasonContent)
        apiMessage.reasonContent = JSON.stringify(lastNodeOutput.reasonContent);
    if (chatflow.followUpPrompts) {
        const followUpPromptsConfig = JSON.parse(chatflow.followUpPrompts);
        const followUpPrompts = await (0, accelance_components_1.generateFollowUpPrompts)(followUpPromptsConfig, apiMessage.content, {
            orgId,
            workspaceId,
            chatId,
            chatflowid,
