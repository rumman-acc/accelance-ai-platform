...
exports.getOAuth2AllowedDomains = getOAuth2AllowedDomains;
exports.validateOAuth2Url = validateOAuth2Url;
exports.extractOAuth2TokenFields = extractOAuth2TokenFields;
...
function getOAuth2AllowedDomains() {
    ...
    if (securityCheckEnabled) {
        return [...new Set([...constants_1.DEFAULT_ALLOWED_OAUTH2_DOMAINS, ...customDomains])];
    }
 ...
 */
function validateOAuth2Url(url) {
    const securityCheckEnabled = process.env.OAUTH2_SECURITY_CHECK !== 'false';
    const allowedDomains = getOAuth2AllowedDomains();
    if (!securityCheckEnabled && allowedDomains.length === 0)
        return;
 ...
 */
function extractOAuth2TokenFields(data) {
    if (!data || typeof data !== 'object' || Array.isArray(data)) {
        throw new Error('Invalid OAuth2 token response: expected an object');
    }
    const result = {};
    for (const key of constants_1.ALLOWED_OAUTH2_TOKEN_FIELDS) {
...
//# sourceMappingURL=oauth2Security.js.map