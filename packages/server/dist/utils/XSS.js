...
exports.getAllowCredentials = getAllowCredentials;
exports.getAllowedAuthCorsOrigins = getAllowedAuthCorsOrigins;
...
exports.getIframeSecurityHeaders = getIframeSecurityHeaders;
const sanitize_html_1 = __importDefault(require("sanitize-html"));
const domainValidation_1 = require("./domainValidation");
...
function validateCorsConfig() {
    if (getAllowedCorsOrigins() === '*' && getAllowCredentials()) {
        logger_1.default.warn('[CORS] Unsafe configuration detected: CORS_ORIGINS=* cannot be combined with ' +
...
function getCorsOptions() {
    return (req, callback) => {
        ...
        const corsOptions = {
            credentials,
            origin: async (origin, originCallback) => {
                const isPublicChatflowReq = (0, domainValidation_1.isPublicChatflowRequest)(req.url);
                const isTTSReq = (0, domainValidation_1.isTTSGenerateRequest)(req.url);
                ...
                if (isPublicChatflowReq || isTTSReq) {
                    ...
                    const chatflowId = isTTSReq ? req.body?.chatflowId : (0, domainValidation_1.extractChatflowId)(req.url);
                    let chatflowAllowed = false;
                    if (chatflowId) {
                        try {
                            chatflowAllowed = await (0, domainValidation_1.validateChatflowDomain)(chatflowId, originLc, req.user?.activeWorkspaceId);
                        }
                        catch (error) {
