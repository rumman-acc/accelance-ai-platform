...
describe('getCorsOptions', () => {
    ...
    afterEach(() => {
        if (SAVED_CORS_ORIGINS !== undefined)
            process.env.CORS_ORIGINS = SAVED_CORS_ORIGINS;
        else
            delete process.env.CORS_ORIGINS;
        if (SAVED_CORS_ALLOW_CREDENTIALS !== undefined)
            process.env.CORS_ALLOW_CREDENTIALS = SAVED_CORS_ALLOW_CREDENTIALS;
        else
            delete process.env.CORS_ALLOW_CREDENTIALS;
    });
    function getCredentials(corsOrigins, corsAllowCredentials) {
        ...
        if (corsAllowCredentials === undefined)
            ...
            process.env.CORS_ALLOW_CREDENTIALS = corsAllowCredentials;
        let captured;
        (0, XSS_1.getCorsOptions)()({ url: '/api/v1/test' }, (_err, options) => {
            captured = options;
        });
    ...
    describe('validateCorsConfig', () => {
        ...
        it('warns when CORS_ORIGINS=* and CORS_ALLOW_CREDENTIALS=true', () => {
            ...
            expect(logger_1.default.warn).toHaveBeenCalledWith(expect.stringContaining('[CORS]'));
        });
...
describe('getAllowedIframeOrigins', () => {
    ...
    describe('default behavior', () => {
        it("should return 'self' when IFRAME_ORIGINS is not set", () => {
            delete process.env.IFRAME_ORIGINS;
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe("'self'");
        });
    ...
    describe('CSP special values', () => {
        ...
        it("should handle 'none' value", () => {
            ...
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe("'none'");
        });
        it('should handle wildcard * value', () => {
            process.env.IFRAME_ORIGINS = '*';
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe('*');
        });
    });
    describe('single domain', () => {
        it('should handle a single FQDN', () => {
            process.env.IFRAME_ORIGINS = 'https://example.com';
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe('https://example.com');
        });
    ...
    describe('multiple domains', () => {
        it('should convert comma-separated domains to space-separated', () => {
            ...
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe('https://domain1.com https://domain2.com');
        });
        it('should handle multiple domains with spaces', () => {
            ...
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe('https://domain1.com https://domain2.com https://domain3.com');
        });
        it('should trim individual domains in comma-separated list', () => {
            process.env.IFRAME_ORIGINS = '  https://app.com  ,  https://admin.com  ';
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe('https://app.com https://admin.com');
        });
    });
    describe('edge cases', () => {
        it('should handle domains with ports', () => {
            process.env.IFRAME_ORIGINS = 'https://localhost:3000,https://localhost:4000';
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe('https://localhost:3000 https://localhost:4000');
        });
        it('should handle domains with paths (though not typical for CSP)', () => {
            ...
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe('https://example.com/path1 https://example.com/path2');
        });
        it('should handle mixed protocols', () => {
            process.env.IFRAME_ORIGINS = 'http://example.com,https://secure.com';
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe('http://example.com https://secure.com');
        });
    ...
    describe('real-world scenarios', () => {
        it('should handle typical production configuration', () => {
            ...
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe('https://app.example.com https://admin.example.com https://dashboard.example.com');
        });
        it('should handle development configuration with localhost', () => {
            ...
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe('http://localhost:3000 http://localhost:3001 http://127.0.0.1:3000');
        });
        it("should handle mix of 'self' and domains", () => {
            process.env.IFRAME_ORIGINS = "'self',https://trusted.com";
            expect((0, XSS_1.getAllowedIframeOrigins)()).toBe("'self' https://trusted.com");
        });
    });
});
describe('getIframeSecurityHeaders', () => {
    ...
    it('returns CSP only for wildcard allowlists', () => {
        process.env.IFRAME_ORIGINS = '*';
        expect((0, XSS_1.getIframeSecurityHeaders)()).toEqual({
