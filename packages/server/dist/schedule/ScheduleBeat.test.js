...
// ─── Imports (after mocks) ────────────────────────────────────────────────────
const ScheduleBeat_1 = require("./ScheduleBeat");
...
const mockGetEnabledSchedulesBatch = schedule_1.default.getEnabledSchedulesBatch;
const mockCronValidate = node_cron_1.default.validate;
const mockCronSchedule = node_cron_1.default.schedule;
// ─── Helpers ──────────────────────────────────────────────────────────────────
...
function resetSingleton(mode) {
    ;
    ScheduleBeat_1.ScheduleBeat.instance = undefined;
...
beforeEach(() => {
    ...
    QueueManager_1.QueueManager.getInstance.mockReturnValue({
        getQueue: jest.fn().mockReturnValue(mockScheduleQueue)
    });
...
describe('getInstance', () => {
    it('returns the same instance on repeated calls', () => {
        const a = ScheduleBeat_1.ScheduleBeat.getInstance();
        const b = ScheduleBeat_1.ScheduleBeat.getInstance();
        expect(a).toBe(b);
    });
...
describe('onScheduleChanged — delete action', () => {
    it('removes a registered cron job (non-queue mode)', async () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        beat.cronJobs.set('rec-1', mockTask);
        await beat.onScheduleChanged('rec-1', 'delete');
        expect(mockTask.stop).toHaveBeenCalled();
        expect(beat.cronJobs.has('rec-1')).toBe(false);
    });
...
describe('shutdown', () => {
    it('stops all registered cron tasks', async () => {
        ...
        beat.cronJobs.set('rec-1', task1);
        beat.cronJobs.set('rec-2', task2);
        await beat.shutdown();
    ...
    it('clears the cronJobs map after shutdown', async () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        beat.cronJobs.set('rec-1', { stop: jest.fn() });
        await beat.shutdown();
        expect(beat.cronJobs.size).toBe(0);
    });
...
describe('_syncAllJobs — non-queue mode', () => {
    it('stops and clears existing cron jobs before syncing', async () => {
        ...
        beat.cronJobs.set('old-rec', existingTask);
        await beat._syncAllJobs();
        expect(existingTask.stop).toHaveBeenCalled();
        expect(beat.cronJobs.has('old-rec')).toBe(false);
    });
...
describe('_upsertCronJob', () => {
    ...
    it('skips registration when cron expression is invalid', () => {
        ...
        beat._upsertCronJob(makeRecord({ cronExpression: 'not-valid' }));
        expect(mockCronSchedule).not.toHaveBeenCalled();
        expect(beat.cronJobs.has('rec-1')).toBe(false);
    });
    it('defaults timezone to UTC when record.timezone is null', () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        beat._upsertCronJob(makeRecord({ timezone: null }));
    ...
    it('fires _onCronFire when the cron task triggers', async () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        const onCronFire = jest.spyOn(beat, '_onCronFire').mockResolvedValue(undefined);
        beat._upsertCronJob(makeRecord());
    ...
    it('expands `L` in DOM field to `28-31` before handing the cron expression to node-cron', () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        beat._upsertCronJob(makeRecord({ cronExpression: '0 9 L * *' }));
    ...
    it('expands `L` correctly inside a comma-separated DOM list', () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        beat._upsertCronJob(makeRecord({ cronExpression: '0 9 1,15,L * *' }));
    ...
    it('expands `L` correctly when the last day of the month is specified and L special character', () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        beat._upsertCronJob(makeRecord({ cronExpression: '0 9 31,L * *' }));
    ...
    it('does not apply runtime DOM filtering when the original expression has no L', () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        const onCronFire = jest.spyOn(beat, '_onCronFire').mockResolvedValue(undefined);
        beat._upsertCronJob(makeRecord({ cronExpression: '0 9 * * 1-5' }));
        const cronCallback = mockCronSchedule.mock.calls[0][1];
    ...
    it('passes the schedule timezone through to the runtime DOM filter', () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        const onCronFire = jest.spyOn(beat, '_onCronFire').mockResolvedValue(undefined);
        beat._upsertCronJob(makeRecord({ cronExpression: '0 9 L * *', timezone: 'America/New_York' }));
...
describe('_removeCronJob', () => {
    it('stops and removes an existing cron task', () => {
        ...
        beat.cronJobs.set('rec-1', task);
        beat._removeCronJob('rec-1');
        expect(task.stop).toHaveBeenCalled();
        expect(beat.cronJobs.has('rec-1')).toBe(false);
    });
    it('is a no-op when the record has no registered task', () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        expect(() => beat._removeCronJob('nonexistent')).not.toThrow();
    });
});
// ─── _onCronFire ──────────────────────────────────────────────────────────────
describe('_onCronFire', () => {
    ...
    it('uses record.id (not the fired schedule id) when removing job in onRecordExpiredOrInvalid', async () => {
        const beat = ScheduleBeat_1.ScheduleBeat.getInstance();
        const task = { stop: jest.fn() };
        beat.cronJobs.set('rec-different', task);
        const record = makeRecord({ id: 'rec-different', enabled: true });
        let capturedCallbacks;
        mockExecuteScheduleJob.mockImplementation(async (_ctx, _id, callbacks) => {
            capturedCallbacks = callbacks;
        });
        await beat._onCronFire('rec-different');
        await capturedCallbacks.onRecordExpiredOrInvalid(record);
        expect(task.stop).toHaveBeenCalled();
    });
});
//# sourceMappingURL=ScheduleBeat.test.js.map