...
const tenantRequestGuards_1 = require("../../enterprise/utils/tenantRequestGuards");
const internalAccelanceError_1 = require("../../errors/internalAccelanceError");
const utils_1 = require("../../utils");
const getRunningExpressApp_1 = require("../../utils/getRunningExpressApp");
const oauth2Security_1 = require("../../utils/oauth2Security");
...
router.post('/authorize/:credentialId', async (req, res, next) => {
    try {
        ...
        const workspaceId = (0, tenantRequestGuards_1.getActiveWorkspaceIdForRequest)(req);
        ...
        if (!credential) {
            return next(new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.NOT_FOUND, 'Credential not found'));
        }
        ...
        try {
            (0, oauth2Security_1.validateOAuth2Url)(authorizationUrl);
        }
        catch (err) {
            return res.status(400).json({
                success: false,
        ...
        next(new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `OAuth2 authorization error: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
});
...
router.get('/callback', async (req, res) => {
    try {
        ...
        try {
            (0, oauth2Security_1.validateOAuth2Url)(tokenUrl);
        }
        catch (err) {
            const errorHtml = (0, templates_1.generateErrorPage)('Invalid token endpoint URL', err instanceof Error ? err.message : 'Token endpoint URL is not allowed', 'Please check your credential configuration.');
            res.setHeader('Content-Type', 'text/html');
            return res.status(400).send(errorHtml);
        }
        const defaultRedirectUri = `${req.protocol}://${req.get('host')}/api/v1/oauth2-credential/callback`;
        ...
        const tokenResponse = await (0, accelance_components_1.secureAxiosRequest)({
            method: 'POST',
            url: tokenUrl,
        ...
        if (tokenResponse.status >= 400) {
            const errorHtml = (0, templates_1.generateErrorPage)(tokenResponse.data?.error || 'token_exchange_failed', tokenResponse.data?.error_description || 'Token exchange failed', tokenResponse.data?.error_description ? `Description: ${tokenResponse.data.error_description}` : undefined);
            res.setHeader('Content-Type', 'text/html');
            return res.status(tokenResponse.status).send(errorHtml);
        }
        const tokenData = (0, oauth2Security_1.extractOAuth2TokenFields)(tokenResponse.data);
...
router.post('/refresh/:credentialId', async (req, res, next) => {
    try {
        ...
        next(new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `OAuth2 token refresh error: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
});
