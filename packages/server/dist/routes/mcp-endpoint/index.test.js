...
jest.mock('../../controllers/mcp-endpoint', () => ({
    __esModule: true,
    default: {
        getRateLimiterMiddleware: (_req, _res, next) => next(),
        authenticateToken: (_req, _res, next) => next(),
        handlePost: (_req, res) => res.status(200).json({ ok: true }),
        handleGet: (_req, res) => res.status(200).end(),
        handleSseMessage: (_req, res) => res.status(200).json({ ok: true }),
        handleDelete: (_req, res) => res.status(200).json({ ok: true })
    }
}));
// ---------------------------------------------------------------------------
...
describe('body size limit', () => {
    ...
    it('accepts a payload just under 1 MiB (500 KB)', async () => {
        ...
        const res = await (0, supertest_1.default)(app).post('/mcp/test-flow').set('Content-Type', 'application/json').send(payload);
