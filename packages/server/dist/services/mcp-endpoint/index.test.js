...
jest.mock('../mcp-server/index', () => ({
    __esModule: true,
    default: {
        getChatflowByIdAndVerifyToken: (...args) => mockGetChatflowByIdAndVerifyToken(...args),
        parseMcpConfig: (...args) => mockParseMcpConfig(...args)
    }
}));
// --- Mock: utilities ---
const mockUtilBuildChatflow = jest.fn();
jest.mock('../../utils/buildChatflow', () => ({
    utilBuildChatflow: (...args) => mockUtilBuildChatflow(...args)
}));
const mockCreateMockRequest = jest.fn();
jest.mock('../../utils/mockRequest', () => ({
    createMockRequest: (...args) => mockCreateMockRequest(...args)
}));
...
describe('handleMcpRequest', () => {
    it('returns 401 when getChatflowByIdAndVerifyToken throws UNAUTHORIZED', async () => {
        mockGetChatflowByIdAndVerifyToken.mockRejectedValue(new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.UNAUTHORIZED, 'Invalid token'));
        const res = makeRes();
        await _1.default.handleMcpRequest('flow-123', 'bad-token', makeReq(), res);
    ...
    it('returns 404 when getChatflowByIdAndVerifyToken throws NOT_FOUND', async () => {
        mockGetChatflowByIdAndVerifyToken.mockRejectedValue(new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.NOT_FOUND, 'Not found'));
        const res = makeRes();
        await _1.default.handleMcpRequest('flow-123', 'token', makeReq(), res);
        expect(res.status).toHaveBeenCalledWith(404);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ jsonrpc: '2.0', error: expect.objectContaining({ code: -32001 }) }));
    });
    it('rethrows non-InternalAccelanceError auth errors', async () => {
        const dbError = new Error('DB connection failed');
        mockGetChatflowByIdAndVerifyToken.mockRejectedValue(dbError);
        await expect(_1.default.handleMcpRequest('flow-123', 'token', makeReq(), makeRes())).rejects.toThrow('DB connection failed');
    });
    ...
    it('calls mcpServer.connect and transport.handleRequest on success', async () => {
        const req = makeReq();
        const res = makeRes();
        await _1.default.handleMcpRequest('flow-123', 'token', req, res);
    ...
    it('registers the tool with toolName and description from config', async () => {
        ...
        await _1.default.handleMcpRequest('flow-123', 'token', makeReq(), makeRes());
    ...
    it('sanitizes chatflow name as toolName when config has no toolName', async () => {
        ...
        await _1.default.handleMcpRequest('flow-123', 'token', makeReq(), makeRes());
        const registeredName = mockMcpTool.mock.calls[0][0];
    ...
    it('uses default description when config.description is absent', async () => {
        ...
        await _1.default.handleMcpRequest('flow-123', 'token', makeReq(), makeRes());
        const registeredDesc = mockMcpTool.mock.calls[0][1];
    ...
    describe('input schema selection', () => {
        it('uses optional question schema for AGENTFLOW type', async () => {
            ...
            await _1.default.handleMcpRequest('flow-123', 'token', makeReq(), makeRes());
            const schema = mockMcpTool.mock.calls[0][2];
    ...
    describe('buildFormInputSchema', () => {
        function makeAgentflowWithFormInputs(formInputTypes) {
            return makeChatflow({
                type: 'AGENTFLOW',
        ...
        it('throws an error when form input configuration is invalid', async () => {
            ...
            mockGetChatflowByIdAndVerifyToken.mockResolvedValue(chatflow);
            await expect(_1.default.handleMcpRequest('flow-123', 'token', makeReq(), makeRes())).rejects.toThrow('Failed to build form input schema due to invalid configuration');
        });
...
describe('chatflow tool callback', () => {
    ...
    it('calls createMockRequest with the chatflowId and question', async () => {
        ...
        expect(mockCreateMockRequest).toHaveBeenCalledWith(expect.objectContaining({
            chatflowId: 'flow-123',
...
describe('handleMcpDeleteRequest', () => {
    it('always returns 405 (session termination not supported in stateless mode)', async () => {
        const res = makeRes();
        await _1.default.handleMcpDeleteRequest('flow-123', makeReq(), res);
    ...
    it('returns 405 regardless of chatflowId', async () => {
        const res = makeRes();
        await _1.default.handleMcpDeleteRequest('any-flow-id', makeReq(), res);
        expect(res.status).toHaveBeenCalledWith(405);
    });
});
