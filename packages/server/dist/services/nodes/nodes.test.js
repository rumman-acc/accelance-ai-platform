...
describe('filterNodeByClient', () => {
    describe('when no client is provided', () => {
        it('returns the node unchanged', () => {
            ...
            const result = (0, filterNodeByClient_1.filterNodeByClient)(node);
    ...
    describe('when client is agentflowsdk', () => {
        it('removes params scoped to agentflowv2 only', () => {
            ...
            const result = (0, filterNodeByClient_1.filterNodeByClient)(node, 'agentflowsdk');
    ...
    describe('when client is agentflowv2', () => {
        it('removes params scoped to agentflowsdk only', () => {
            ...
            const result = (0, filterNodeByClient_1.filterNodeByClient)(node, 'agentflowv2');
            expect(result.inputs).toHaveLength(1);
            expect(result.inputs[0].name).toBe('startEphemeralMemory');
        });
    ...
    describe('Start node form input filtering', () => {
        ...
        it('removes formInput option for agentflowsdk', () => {
            const result = (0, filterNodeByClient_1.filterNodeByClient)(startNode, 'agentflowsdk');
            const optionNames = result.inputs[0].options.map((o) => o.name);
        ...
        it('keeps formInput option for agentflowv2', () => {
            const result = (0, filterNodeByClient_1.filterNodeByClient)(startNode, 'agentflowv2');
            const optionNames = result.inputs[0].options.map((o) => o.name);
            expect(optionNames).toContain('chatInput');
    ...
    describe('Agent node knowledge field filtering', () => {
        ...
        it('removes knowledge fields for agentflowsdk', () => {
            const result = (0, filterNodeByClient_1.filterNodeByClient)(agentNode, 'agentflowsdk');
        ...
        it('keeps knowledge fields for agentflowv2', () => {
            const result = (0, filterNodeByClient_1.filterNodeByClient)(agentNode, 'agentflowv2');
            const inputNames = result.inputs.map((i) => i.name);
            expect(inputNames).toContain('agentName');
