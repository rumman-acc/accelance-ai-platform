...
jest.mock('../../utils', () => ({
    encryptCredentialData: jest.fn((data) => Promise.resolve(`encrypted:${JSON.stringify(data)}`)),
    decryptCredentialData: jest.fn((data) => {
        if (data.startsWith('encrypted:'))
            return Promise.resolve(JSON.parse(data.slice('encrypted:'.length)));
...
jest.mock('accelance-components', () => ({
    ...
    checkDenyList: (url) => mockCheckDenyList(url),
    isValidURL: (url) => {
        try {
            new URL(url);
            return true;
    ...
    validateCustomHeaders: (headers) => mockValidateCustomHeaders(headers)
}));
...
const mockEncrypt = utils_1.encryptCredentialData;
const mockDecrypt = utils_1.decryptCredentialData;
// ── Helpers ─────────────────────────────────────────────────────────────────
...
describe('customMcpServersService', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        mockCreate.mockImplementation((entity) => entity);
    });
    ...
    describe('createCustomMcpServer', () => {
        it('should create a record and return db response', async () => {
            ...
            const result = await index_1.default.createCustomMcpServer({ name: 'Test Server', serverUrl: 'https://api.example.com' }, 'org-1');
            expect(mockSave).toHaveBeenCalled();
            expect(mockSendTelemetry).toHaveBeenCalledWith('custom_mcp_server_created', expect.objectContaining({ version: '1.0.0', toolId: 'mcp-1', toolName: 'Test Server' }), 'org-1');
            expect(mockIncrementCounter).toHaveBeenCalledWith(Interface_Metrics_1.ACCELANCE_METRIC_COUNTERS.CUSTOM_MCP_SERVER_CREATED, {
        ...
        it('should encrypt authConfig when it is an object', async () => {
            const saved = makeRecord();
            mockSave.mockResolvedValue(saved);
            await index_1.default.createCustomMcpServer({ name: 'Test', serverUrl: 'https://example.com', authConfig: { headers: { 'X-Key': 'secret' } } }, 'org-1');
        ...
        it('should not encrypt authConfig when it is not an object', async () => {
            const saved = makeRecord();
            mockSave.mockResolvedValue(saved);
            await index_1.default.createCustomMcpServer({ name: 'Test', serverUrl: 'https://example.com', authConfig: null }, 'org-1');
            expect(mockEncrypt).not.toHaveBeenCalled();
        });
        it('should validate serverUrl and throw for invalid URL', async () => {
            await expect(index_1.default.createCustomMcpServer({ name: 'Bad', serverUrl: 'not-a-url' }, 'org-1')).rejects.toThrow(internalAccelanceError_1.InternalAccelanceError);
            await expect(index_1.default.createCustomMcpServer({ name: 'Bad', serverUrl: 'not-a-url' }, 'org-1')).rejects.toThrow('not a valid URL');
        });
        it('should validate serverUrl and throw for non-http protocol', async () => {
            await expect(index_1.default.createCustomMcpServer({ name: 'Bad', serverUrl: 'ftp://example.com' }, 'org-1')).rejects.toThrow('only http and https are allowed');
        });
        it('should re-throw InternalAccelanceError as-is', async () => {
            const specificError = new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'custom error');
            mockSave.mockRejectedValue(specificError);
            await expect(index_1.default.createCustomMcpServer({ name: 'Test', serverUrl: 'https://example.com' }, 'org-1')).rejects.toThrow(specificError);
        });
        it('should wrap unknown errors in InternalAccelanceError', async () => {
            mockSave.mockRejectedValue(new Error('db crash'));
            await expect(index_1.default.createCustomMcpServer({ name: 'Test', serverUrl: 'https://example.com' }, 'org-1')).rejects.toThrow(internalAccelanceError_1.InternalAccelanceError);
        });
    });
    ...
    describe('getAllCustomMcpServers', () => {
        it('should return sanitized array when no pagination', async () => {
            ...
            const result = await index_1.default.getAllCustomMcpServers('ws-1');
        ...
        it('should return paginated result when page and limit are positive', async () => {
            ...
            const result = await index_1.default.getAllCustomMcpServers('ws-1', 2, 5);
        ...
        it('should filter by workspaceId when provided', async () => {
            ...
            await index_1.default.getAllCustomMcpServers('ws-1');
        ...
        it('should wrap errors in InternalAccelanceError', async () => {
            ...
            await expect(index_1.default.getAllCustomMcpServers('ws-1')).rejects.toThrow(internalAccelanceError_1.InternalAccelanceError);
        });
    });
    ...
    describe('getCustomMcpServerById', () => {
        it('should return masked response without authConfig when no authConfig exists', async () => {
            ...
            const result = await index_1.default.getCustomMcpServerById('mcp-1', 'ws-1');
        ...
        it('should throw NOT_FOUND when record does not exist', async () => {
            ...
            await expect(index_1.default.getCustomMcpServerById('mcp-1', 'ws-1')).rejects.toThrow(internalAccelanceError_1.InternalAccelanceError);
            await expect(index_1.default.getCustomMcpServerById('mcp-1', 'ws-1')).rejects.toThrow('not found');
        });
        it('should wrap unknown errors in InternalAccelanceError', async () => {
    ...
    describe('updateCustomMcpServer', () => {
        it('should update and return the record', async () => {
            ...
            const result = await index_1.default.updateCustomMcpServer('mcp-1', { name: 'Updated' }, 'ws-1');
        ...
        it('should throw NOT_FOUND when record does not exist', async () => {
            ...
            await expect(index_1.default.updateCustomMcpServer('mcp-1', { name: 'X' }, 'ws-1')).rejects.toThrow('not found');
        });
        it('should preserve real serverUrl when client sends masked value', async () => {
            ...
            mockFindOneBy.mockResolvedValue(existing);
            mockSave.mockImplementation((r) => Promise.resolve(r));
            const body = { serverUrl: 'https://real.example.com/************' };
            await index_1.default.updateCustomMcpServer('mcp-1', body, 'ws-1');
        ...
        it('should validate new serverUrl when client sends a non-masked value', async () => {
            const existing = makeRecord();
            mockFindOneBy.mockResolvedValue(existing);
            await expect(index_1.default.updateCustomMcpServer('mcp-1', { serverUrl: 'ftp://bad.com' }, 'ws-1')).rejects.toThrow('only http and https are allowed');
        });
        it('should reject URL that partially contains the mask placeholder', async () => {
            ...
            const existing = makeRecord({ serverUrl: 'https://real.example.com/secret' });
            mockFindOneBy.mockResolvedValue(existing);
            await expect(index_1.default.updateCustomMcpServer('mcp-1', { serverUrl: 'https://typo-host.com/************' }, // new host but mask still present
            'ws-1')).rejects.toThrow(/masked placeholder/);
        });
        it('should encrypt authConfig on update', async () => {
            const existing = makeRecord();
            mockFindOneBy.mockResolvedValue(existing);
            mockSave.mockImplementation((r) => Promise.resolve(r));
            await index_1.default.updateCustomMcpServer('mcp-1', { authConfig: { headers: { 'X-Key': 'new' } } }, 'ws-1');
        ...
        it('should reject header value that partially contains the mask placeholder', async () => {
            ...
            await expect(index_1.default.updateCustomMcpServer('mcp-1', body, 'ws-1')).rejects.toThrow(/masked placeholder/);
        });
        ...
        it('should force workspaceId on saved record (defense-in-depth)', async () => {
            ...
            mockFindOneBy.mockResolvedValue(existing);
            mockSave.mockImplementation((r) => Promise.resolve(r));
            await index_1.default.updateCustomMcpServer('mcp-1', { name: 'Updated' }, 'ws-1');
    ...
    describe('deleteCustomMcpServer', () => {
        it('should delete by id and workspaceId', async () => {
            ...
            const result = await index_1.default.deleteCustomMcpServer('mcp-1', 'ws-1');
            expect(mockDelete).toHaveBeenCalledWith({ id: 'mcp-1', workspaceId: 'ws-1' });
            expect(result).toEqual({ affected: 1 });
        });
        it('should wrap errors in InternalAccelanceError', async () => {
            mockDelete.mockRejectedValue(new Error('db error'));
            await expect(index_1.default.deleteCustomMcpServer('mcp-1', 'ws-1')).rejects.toThrow(internalAccelanceError_1.InternalAccelanceError);
        });
    });
    ...
    describe('authorizeCustomMcpServer', () => {
        it('should throw NOT_FOUND when record does not exist', async () => {
            mockFindOneBy.mockResolvedValue(null);
            await expect(index_1.default.authorizeCustomMcpServer('mcp-1', 'ws-1')).rejects.toThrow('not found');
        });
        it('should connect, discover tools, set AUTHORIZED status, and return record', async () => {
            ...
            mockSave.mockImplementation((r) => Promise.resolve(r));
            const result = await index_1.default.authorizeCustomMcpServer('mcp-1', 'ws-1');
            expect(accelance_components_1.MCPToolkit).toHaveBeenCalledWith({ url: 'https://example.com/mcp' }, 'sse');
            expect(mockInitialize).toHaveBeenCalled();
            expect(result.status).toBe(Interface_1.CustomMcpServerStatus.AUTHORIZED);
        ...
        it('should include decrypted headers in server params when authType is CUSTOM_HEADERS', async () => {
            ...
            mockInitialize.mockResolvedValue(undefined);
            mockSave.mockImplementation((r) => Promise.resolve(r));
            await index_1.default.authorizeCustomMcpServer('mcp-1', 'ws-1');
            expect(accelance_components_1.MCPToolkit).toHaveBeenCalledWith({ url: 'https://example.com', headers: { 'X-Api-Key': 'my-secret' } }, 'sse');
        });
        it('should not include headers when authType is NONE even if authConfig is present', async () => {
            ...
            mockSave.mockImplementation((r) => Promise.resolve(r));
            await index_1.default.authorizeCustomMcpServer('mcp-1', 'ws-1');
            expect(accelance_components_1.MCPToolkit).toHaveBeenCalledWith({ url: 'https://example.com' }, 'sse');
        ...
        it('should set ERROR status and throw when connection fails', async () => {
            ...
            mockSave.mockImplementation((r) => Promise.resolve(r));
            await expect(index_1.default.authorizeCustomMcpServer('mcp-1', 'ws-1')).rejects.toThrow('Failed to connect to Custom MCP server');
            expect(record.status).toBe(Interface_1.CustomMcpServerStatus.ERROR);
            expect(mockSave).toHaveBeenCalled();
        });
        it('should close toolkit client even on failure', async () => {
            ...
            mockSave.mockImplementation((r) => Promise.resolve(r));
            await expect(index_1.default.authorizeCustomMcpServer('mcp-1', 'ws-1')).rejects.toThrow();
            expect(mockClose).toHaveBeenCalled();
        });
    });
    // ── getDiscoveredTools ──────────────────────────────────────────────
    describe('getDiscoveredTools', () => {
        it('should throw NOT_FOUND when record does not exist', async () => {
            mockGetOne.mockResolvedValue(null);
            await expect(index_1.default.getDiscoveredTools('mcp-1', 'ws-1')).rejects.toThrow('not found');
        });
        it('should return empty array when tools is null', async () => {
            mockGetOne.mockResolvedValue(makeRecord({ tools: null }));
            const result = await index_1.default.getDiscoveredTools('mcp-1', 'ws-1');
    ...
    describe('SSRF denylist enforcement', () => {
        ...
        it('rejects create when checkDenyList throws (private IP / loopback / IMDS)', async () => {
            ...
            await expect(index_1.default.createCustomMcpServer({ name: 'T', serverUrl: 'http://169.254.169.254/' }, 'org-1')).rejects.toThrow('Server URL is not allowed by policy');
        });
        it('rejects update when a new non-masked URL is denied', async () => {
            mockFindOneBy.mockResolvedValue(makeRecord());
            mockCheckDenyList.mockRejectedValueOnce(new Error('denied'));
            await expect(index_1.default.updateCustomMcpServer('mcp-1', { serverUrl: 'http://127.0.0.1:6379' }, 'ws-1')).rejects.toThrow('Server URL is not allowed by policy');
        });
        it('does not call checkDenyList when update sends a masked URL', async () => {
            ...
            mockSave.mockImplementation((r) => Promise.resolve(r));
            await index_1.default.updateCustomMcpServer('mcp-1', { serverUrl: 'https://real.example.com/************' }, 'ws-1');
    ...
    describe('header validation enforcement', () => {
        ...
        it('runs validateCustomHeaders on create when authType is CUSTOM_HEADERS', async () => {
            ...
            await index_1.default.createCustomMcpServer({
                name: 'T',
        ...
        it('maps validator error to BAD_REQUEST', async () => {
            ...
            await expect(index_1.default.createCustomMcpServer({
                name: 'T',
                serverUrl: 'https://example.com',
                authType: 'CUSTOM_HEADERS',
                authConfig: { headers: { Host: 'evil' } }
            }, 'org-1')).rejects.toThrow('Invalid header "Host": not allowed');
        });
    });
    describe('tools payload cap', () => {
        ...
        afterEach(() => {
            if (ORIGINAL_ENV === undefined)
                delete process.env.CUSTOM_MCP_TOOLS_MAX_BYTES;
            else
                process.env.CUSTOM_MCP_TOOLS_MAX_BYTES = ORIGINAL_ENV;
        });
        it('rejects authorize when serialized tools exceed the limit and marks row ERROR', async () => {
            ...
            await expect(index_1.default.authorizeCustomMcpServer('mcp-1', 'ws-1')).rejects.toThrow(/tools payload larger than the allowed limit/);
            expect(record.status).toBe(Interface_1.CustomMcpServerStatus.ERROR);
        });
    ...
    describe('authorize handshake timeout', () => {
        ...
        afterEach(() => {
            if (ORIGINAL_ENV === undefined)
                delete process.env.CUSTOM_MCP_AUTHORIZE_TIMEOUT_MS;
            else
                process.env.CUSTOM_MCP_AUTHORIZE_TIMEOUT_MS = ORIGINAL_ENV;
        });
        it('rejects and marks ERROR when initialize() hangs past the timeout', async () => {
            ...
            mockSave.mockImplementation((r) => Promise.resolve(r));
            await expect(index_1.default.authorizeCustomMcpServer('mcp-1', 'ws-1')).rejects.toThrow(/exceeded 1000ms/);
            expect(record.status).toBe(Interface_1.CustomMcpServerStatus.ERROR);
