...
// Helpers
// ---------------------------------------------------------------------------
const makeNode = (id, name, inputParams = [], inputs = {}, label) => ({
    id,
...
const emptyComponentNodes = {};
const issuesFor = (results, nodeId) => {
...
describe('validateFlowData', () => {
    it('returns empty array for a valid connected flow', () => {
        ...
        const results = (0, index_1.validateFlowData)(nodes, edges, emptyComponentNodes);
    ...
    it('skips stickyNoteAgentflow nodes', () => {
        ...
        const results = (0, index_1.validateFlowData)(nodes, [], emptyComponentNodes);
        expect(results).toEqual([]);
    });
    // --- Required parameters ---
    ...
    it('skips validation when show condition is not met', () => {
        const nodes = [
            makeNode('n1', 'api', [{ name: 'body', label: 'Body', show: { bodyType: 'raw' } }], { bodyType: 'formData' } // show condition NOT met
            )
        ];
        const edges = [makeEdge('n1', 'n1')];
        const results = (0, index_1.validateFlowData)(nodes, edges, emptyComponentNodes);
    ...
    it('flags missing required nested config fields', () => {
        ...
        const edges = [makeEdge('n1', 'n1')];
        const results = (0, index_1.validateFlowData)(nodes, edges, componentNodes);
...
//# sourceMappingURL=validateFlowData.test.js.map