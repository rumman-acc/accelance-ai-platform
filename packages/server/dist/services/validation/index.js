...
exports.validateFlowData = void 0;
 ...
 */
const validateFlowData = (nodes, edges, componentNodes) => {
    ...
    for (const node of nodes) {
                            ...
                            if (param.array) {
                                param.array.forEach((arrayParam) => {
                                    ...
                                    if (shouldValidate) {
                                        ...
                                        if ((arrayParam.optional === undefined || !arrayParam.optional) &&
        ...
        if (node.data.name === 'startAgentflow' && node.data.inputs?.startInputType === 'scheduleInput') {
            const cronResult = (0, schedule_1.resolveScheduleCron)(node.data.inputs);
            if (!cronResult.valid) {
...
const checkFlowValidation = async (flowId, workspaceId) => {
    try {
        const appServer = (0, getRunningExpressApp_1.getRunningExpressApp)();
        // Create query conditions with workspace filtering if provided
        const whereCondition = { id: flowId };
        if (workspaceId)
            whereCondition.workspaceId = workspaceId;
        const flow = await appServer.AppDataSource.getRepository(ChatFlow_1.ChatFlow).findOne({
            where: whereCondition
        });
        if (!flow) {
            throw new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: validationService.checkFlowValidation - flow not found!`);
        }
        const flowData = JSON.parse(flow.flowData);
        return (0, exports.validateFlowData)(flowData.nodes, flowData.edges, appServer.nodesPool.componentNodes);
    }
    catch (error) {
        throw new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: validationService.checkFlowValidation - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
exports.default = {
    checkFlowValidation
