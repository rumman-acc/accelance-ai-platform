...
const makeChatflow = (startInputType, inputs, entityFields) => ({
    id: 'test-id',
    ...entityFields,
...
describe('validateWebhookChatflow', () => {
    ...
    it('throws 404 when chatflow is not found', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('missing-id')).rejects.toMatchObject({
    ...
    it('throws 404 when chatflow is not configured as a webhook trigger', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id')).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.NOT_FOUND
        });
    });
    it('resolves without error for a valid webhook chatflow', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id')).resolves.toMatchObject({});
    });
    ...
    it('preserves InternalAccelanceError without wrapping', async () => {
        const original = new internalAccelanceError_1.InternalAccelanceError(http_status_codes_1.StatusCodes.NOT_FOUND, 'already an internal error');
        mockGetChatflowById.mockRejectedValue(original);
        await expect(index_1.default.validateWebhookChatflow('some-id')).rejects.toBe(original);
    });
    // --- Method validation ---
    it('throws 405 when HTTP method does not match configured webhookMethod', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'GET')).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.METHOD_NOT_ALLOWED
        });
    });
    it('resolves for any method when webhookMethod is not configured', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger'));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'DELETE')).resolves.toMatchObject({});
    });
    ...
    it('throws 415 when Content-Type does not match configured webhookContentType', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { foo: 'bar' }, 'POST', { 'content-type': 'text/plain' })).rejects.toMatchObject({ statusCode: http_status_codes_1.StatusCodes.UNSUPPORTED_MEDIA_TYPE });
    });
    it('resolves when Content-Type starts with configured value (handles charset suffix)', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger', { webhookContentType: 'application/json' }));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { foo: 'bar' }, 'POST', {
            'content-type': 'application/json; charset=utf-8'
        })).resolves.toMatchObject({});
    });
    it('resolves when webhookContentType is not configured', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger'));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { foo: 'bar' }, 'POST', { 'content-type': 'text/plain' })).resolves.toMatchObject({});
    });
    it('skips Content-Type check when request has no body (empty POST ping)', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', {})).resolves.toMatchObject({});
    });
    // --- Header validation ---
    it('throws 400 when a required header is missing', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger', { webhookEnableValidation: true, webhookHeaderParams: [{ name: 'x-api-key', required: true }] }));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', {})).rejects.toMatchObject({
    ...
    it('resolves when all required headers are present', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger', { webhookEnableValidation: true, webhookHeaderParams: [{ name: 'x-api-key', required: true }] }));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', { 'x-api-key': 'secret' })).resolves.toMatchObject({});
    });
    // --- Body param validation ---
    it('throws 400 when a required param is missing from body', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger', { webhookEnableValidation: true, webhookBodyParams: [{ name: 'action', required: true }] }));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {})).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST
        });
    });
    ...
    it('resolves when all required params are present in body', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger', { webhookEnableValidation: true, webhookBodyParams: [{ name: 'action', required: true }] }));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { action: 'push' })).resolves.toMatchObject({});
    });
    it('resolves when webhookBodyParams is empty string (DB default)', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {})).resolves.toMatchObject({});
    });
    it('resolves when no params declared but body has arbitrary fields', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger'));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { anything: 'goes' })).resolves.toMatchObject({});
    });
    ...
    it('throws 400 when a declared body param has wrong type', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { count: 'not-a-number' })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
    ...
    it('resolves when declared body param has correct type', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { count: 42 })).resolves.toMatchObject({});
    });
    ...
    it('throws 400 when number param is an empty string (form-encoded)', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { count: '' })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
            message: expect.stringContaining('count')
        });
    });
    it('resolves when boolean param is a native boolean (JSON)', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { active: true })).resolves.toMatchObject({});
    });
    ...
    it('resolves when boolean param is the string "false" (form-encoded)', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { active: 'false' })).resolves.toMatchObject({});
    });
    it('throws 400 when boolean param is an invalid string like "yes" (form-encoded)', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { active: 'yes' })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
    ...
    it('resolves when object param is a plain object', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { meta: { key: 'val' } })).resolves.toMatchObject({});
    });
    it('throws 400 when object param is a string', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { meta: 'hello' })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
    ...
    it('throws 400 when object param is an array', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { meta: [1, 2] })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
            message: expect.stringContaining('meta')
        });
    });
    ...
    it('resolves when array[string] param is an array of strings', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { tags: ['a', 'b'] })).resolves.toMatchObject({});
    });
    it('throws 400 when array[string] param contains non-strings', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { tags: [1, 2] })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
    ...
    it('throws 400 when array[string] param is not an array', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { tags: 'hello' })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
            message: expect.stringContaining('tags')
        });
    });
    ...
    it('resolves when array[number] param is an array of numbers', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { scores: [1, 2, 3] })).resolves.toMatchObject({});
    });
    it('throws 400 when array[number] param contains non-numbers', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { scores: ['a', 'b'] })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
    ...
    it('resolves when array[boolean] param is an array of booleans', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { flags: [true, false] })).resolves.toMatchObject({});
    });
    it('throws 400 when array[boolean] param contains boolean strings (no coercion for array elements)', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { flags: ['true', 'false'] })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
    ...
    it('resolves when array[object] param is an array of plain objects', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { items: [{ a: 1 }, { b: 2 }] })).resolves.toMatchObject({});
    });
    it('throws 400 when array[object] param contains non-objects', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { items: ['a', 'b'] })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
    ...
    it('throws 400 when array[object] param contains nested arrays', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, { items: [[1, 2]] })).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
            message: expect.stringContaining('items')
        });
    });
    ...
    it('throws 400 when a required query param is missing', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger', { webhookEnableValidation: true, webhookQueryParams: [{ name: 'page', required: true }] }));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', {}, {})).rejects.toMatchObject({
            statusCode: http_status_codes_1.StatusCodes.BAD_REQUEST,
    ...
    it('resolves when all required query params are present', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger', { webhookEnableValidation: true, webhookQueryParams: [{ name: 'page', required: true }] }));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', {}, { page: '2' })).resolves.toMatchObject({});
    });
    ...
    it('resolves without signature check when no webhookSecret is configured', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger'));
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', {}, {}, RAW_BODY)).resolves.toMatchObject({});
    });
    it('resolves when secret is set and signature is valid', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger', { webhookEnableAuth: true }, { webhookSecretConfigured: true }));
        mockGetWebhookSecret.mockResolvedValue(SECRET);
        const headers = { 'x-webhook-signature': sign(SECRET, RAW_BODY) };
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', headers, {}, RAW_BODY)).resolves.toMatchObject({});
    });
    it('throws 401 when secret is set but X-Webhook-Signature header is missing', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger', { webhookEnableAuth: true }, { webhookSecretConfigured: true }));
        mockGetWebhookSecret.mockResolvedValue(SECRET);
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', {}, {}, RAW_BODY)).rejects.toMatchObject({
            statusCode: 401,
    ...
    it('throws 500 with config hint when auth is enabled but no secret has been generated', async () => {
        ...
        const headers = { 'x-webhook-signature': sign(SECRET, RAW_BODY) };
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', headers, {}, RAW_BODY)).rejects.toMatchObject({
            statusCode: 500,
    ...
    it('throws 401 when payload is tampered (signature computed against original body)', async () => {
        ...
        const headers = { 'x-webhook-signature': sign(SECRET, RAW_BODY) };
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', headers, {}, tamperedBody)).rejects.toMatchObject({ statusCode: 401 });
    });
    it('throws 401 when secret is set but rawBody is undefined', async () => {
        mockGetChatflowById.mockResolvedValue(makeChatflow('webhookTrigger', { webhookEnableAuth: true }, { webhookSecretConfigured: true }));
        mockGetWebhookSecret.mockResolvedValue(SECRET);
        const headers = { 'x-webhook-signature': sign(SECRET, RAW_BODY) };
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', headers, {}, undefined)).rejects.toMatchObject({ statusCode: 401 });
    });
    ...
    it('skips field validation when skipFieldValidation is true', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', {}, {}, undefined, { skipFieldValidation: true })).resolves.toMatchObject({});
    });
    it('still runs signature check when skipFieldValidation is true', async () => {
        ...
        await expect(index_1.default.validateWebhookChatflow('some-id', undefined, {}, 'POST', {}, {}, RAW_BODY, { skipFieldValidation: true })).rejects.toMatchObject({ statusCode: 401 });
    });
});
