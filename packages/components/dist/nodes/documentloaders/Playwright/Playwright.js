...
class Playwright_DocumentLoaders {
    ...
    async init(nodeData, _, options) {
        ...
        if (relativeLinksMethod) {
            if (process.env.DEBUG === 'true')
                options.logger.info(`[${orgId}]: Start PlaywrightWebBaseLoader ${relativeLinksMethod}`);
            ...
            if (process.env.DEBUG === 'true')
                options.logger.info(`[${orgId}]: Finish PlaywrightWebBaseLoader ${relativeLinksMethod}`);
        }
        else if (selectedLinks && selectedLinks.length > 0) {
            if (process.env.DEBUG === 'true')
                options.logger.info(`[${orgId}]: PlaywrightWebBaseLoader pages: ${JSON.stringify(selectedLinks)}, length: ${selectedLinks.length}`);
