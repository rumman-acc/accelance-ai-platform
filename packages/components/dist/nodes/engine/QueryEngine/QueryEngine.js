...
class QueryEngine_LlamaIndex {
    ...
    async run(nodeData, input, options) {
        ...
        await EvaluationRunTracerLlama_1.EvaluationRunTracerLlama.injectEvaluationMetadata(nodeData, options, queryEngine)
    ...
    if (responseSynthesizerObj) {
        if (responseSynthesizerObj.type === 'TreeSummarize') {
            const responseSynthesizer = new llamaindex_1.ResponseSynthesizer({
                responseBuilder: new llamaindex_1.TreeSummarize(
            ...
            const responseSynthesizer = new llamaindex_1.ResponseSynthesizer({
                responseBuilder: new llamaindex_1.CompactAndRefine(
                    vectorStoreRetriever.serviceContext,
                    responseSynthesizerObj.textQAPromptTemplate,
            ...
            const responseSynthesizer = new llamaindex_1.ResponseSynthesizer({
                responseBuilder: new llamaindex_1.Refine(
                    vectorStoreRetriever.serviceContext,
                    responseSynthesizerObj.textQAPromptTemplate,
                    responseSynthesizerObj.refinePromptTemplate
                ),
