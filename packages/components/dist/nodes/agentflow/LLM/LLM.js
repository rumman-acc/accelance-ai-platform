...
class LLM_Agentflow {
    ...
    async run(nodeData, input, options) {
        ...
        try {
            ...
            if (response.response_metadata) {
                const { artifacts: extractedArtifacts, fileAnnotations: extractedFileAnnotations, savedInlineImages } = await (0, utils_1.extractArtifactsFromResponse)(response.response_metadata, newNodeData, options);
            ...
            let thinkingDuration = typeof response.additional_kwargs?.reasoning_duration === 'number'
            ...
            const output = this.prepareOutputObject(response, finalResponse, startTime, endTime, timeDelta, isStructuredOutput, artifacts, fileAnnotations, reasonContentObj, costMetadata);
     ...
     */
    async handleMemory({ messages, memoryType, pastChatHistory, runtimeChatHistory, llmNodeInstance, nodeData, userMessage, input, abortController, options, modelConfig }) {
     ...
     */
    prepareOutputObject(response, finalResponse, startTime, endTime, timeDelta, isStructuredOutput, artifacts = [], fileAnnotations = [], reasonContent, costMetadata) {
        const output = {
            content: finalResponse,
            timeMetadata: {
                start: startTime,
