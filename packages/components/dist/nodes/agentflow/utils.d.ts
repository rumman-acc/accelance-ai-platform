...
export declare const saveBase64Image: (outputItem: {
    result?: string;
    id?: string;
    output_format?: string;
}, options: ICommonObject) => Promise<ISavedImageResult | null>;
/** Saves a Gemini inline image to storage. */
export declare const saveGeminiInlineImage: (inlineItem: {
    data?: string;
    mimeType?: string;
}, options: ICommonObject) => Promise<ISavedImageResult | null>;
/** Downloads a file from an OpenAI container (used for file citations in responses). */
export declare const downloadContainerFile: (containerId: string, fileId: string, filename: string, modelNodeData: INodeData, options: ICommonObject) => Promise<{
    filePath: string;
    totalSize: number;
} | null>;
/**
 ...
 */
export declare const extractArtifactsFromResponse: (responseMetadata: IResponseMetadata | undefined, modelNodeData: INodeData, options: ICommonObject) => Promise<{
    artifacts: IArtifact[];
