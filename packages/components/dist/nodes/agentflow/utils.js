...
exports.updateFlowState = exports.addImageArtifactsToMessages = exports.extractArtifactsFromResponse = exports.replaceInlineDataWithFileReferences = exports.downloadContainerFile = exports.saveGeminiInlineImage = exports.saveBase64Image = exports.getArtifactTypeFromFilename = exports.getMimeTypeFromFilename = exports.getPastChatHistoryImageMessages = exports.getUniqueImageMessages = exports.normalizeMessagesForStorage = exports.revertBase64ImagesToFileRefs = exports.processMessagesWithImages = void 0;
...
const storedFileToBase64 = async (fileName, mime, options) => {
    const contents = await (0, storageUtils_1.getFileFromStorage)(fileName, options.orgId, options.chatflowid, options.chatId);
...
const saveImageToStorage = async (base64Data, mimeType, fileName, options) => {
    ...
    for (let i = 0; i < updatedMessages.length; i++) {
        ...
        for (const item of message.content) {
            if (item.type === 'stored-file' && item.name && item.mime?.startsWith('image/')) {
                ...
                try {
                    const fileName = (0, validator_1.sanitizeFileName)(item.name);
        ...
        if (imageContents.length > 0) {
            ...
            if (fileRefs.length > 0) {
                if (!updatedMessages[i].additional_kwargs)
                    updatedMessages[i].additional_kwargs = {};
 ...
 */
const revertBase64ImagesToFileRefs = (messages) => {
    const updatedMessages = JSON.parse(JSON.stringify(messages));
    for (const message of updatedMessages) {
        const fileRefs = message.additional_kwargs?._imageFileRefs || [];
        if (message.content && Array.isArray(message.content) && fileRefs.length > 0) {
            ...
            for (const ref of fileRefs) {
                const item = contentArray[ref.index];
...
exports.revertBase64ImagesToFileRefs = revertBase64ImagesToFileRefs;
/**
...
exports.normalizeMessagesForStorage = normalizeMessagesForStorage;
...
const getUniqueImageMessages = async (options, messages, modelConfig) => {
    ...
    const alreadyPresentRefs = new Set();
    for (const msg of messages) {
        const refs = msg.additional_kwargs?._imageFileRefs;
        if (refs) {
            for (const r of refs) {
                alreadyPresentRefs.add(`${(0, validator_1.sanitizeFileName)(r.fileName)}:${r.mime}`);
            }
        }
    }
    ...
    images.forEach((image, index) => {
        ...
        if (upload && alreadyPresentRefs.has(`${(0, validator_1.sanitizeFileName)(upload.name)}:${upload.mime}`)) {
            return;
        }
        const alreadyInContent = messages.some((msg) => {
            const chatMsg = msg;
            if (Array.isArray(chatMsg.content)) {
                return chatMsg.content.some((item) => item.type === 'image_url' &&
    ...
    const imageMessageWithFileRef = {
        ...
        content: uniquePairs.map(({ upload }) => ({
            type: upload.type,
            name: (0, validator_1.sanitizeFileName)(upload.name),
            mime: upload.mime
        }))
    };
    ...
    uniquePairs.forEach(({ upload }, i) => {
        if (upload && upload.type === 'stored-file') {
            fileRefs.push({ index: i, fileName: (0, validator_1.sanitizeFileName)(upload.name), mime: upload.mime });
        }
    });
    // Base64 version: full image data for model invocation
    const imageMessageWithBase64 = { role: 'user', content: uniqueImages };
    if (fileRefs.length > 0) {
...
const getPastChatHistoryImageMessages = async (pastChatHistory, options) => {
    ...
    for (let i = 0; i < pastChatHistory.length; i++) {
        ...
        const collectKwargs = (source) => {
            const result = {};
            let found = false;
            for (const key of ['artifacts', 'fileAnnotations', 'usedTools']) {
        ...
        if (message.additional_kwargs && message.additional_kwargs.fileUploads) {
            ...
            try {
                ...
                for (const upload of uploads) {
                    if (upload.type === 'stored-file' && upload.mime.startsWith('image/')) {
                        ...
                        const fileName = (0, validator_1.sanitizeFileName)(upload.name);
                        ...
                        const safeFileName = (0, validator_1.sanitizeFileName)(upload.name);
                        const fileLoaderNodeModule = await Promise.resolve().then(() => __importStar(require('../../nodes/documentloaders/File/File')));
                        // @ts-ignore
                        const fileLoaderNodeInstance = new fileLoaderNodeModule.nodeClass();
                        const nodeData = {
                            inputs: {
                                [(0, utils_1.mapMimeTypeToInputField)(upload.mime)]: `FILE-STORAGE::${JSON.stringify([safeFileName])}`
                            }
                        };
                        const documents = await fileLoaderNodeInstance.init(nodeData, '', {
                            retrieveAttachmentChatId: true,
                            chatflowid: options.chatflowid,
                            chatId: options.chatId,
                            orgId: options.orgId
                        });
                        messageWithFileUploads += `<doc name='${safeFileName}'>${(0, utils_1.handleEscapeCharacters)(documents, true)}</doc>\n\n`;
                    }
                }
                const extraKwargs = collectKwargs({ artifacts, fileAnnotations, usedTools });
                ...
                if (imageContents.length > 0) {
                    ...
                    const rawFileUploads = pastChatHistory[i].additional_kwargs.fileUploads;
                    transformedPastMessages.push({
                        role: messageRole,
                        content: [...JSON.parse(rawFileUploads)]
                    });
                }
                ...
                const messageContent = messageWithFileUploads
                    ...
                    : message.content;
                const textMsg = { role: messageRole, content: messageContent };
                if (extraKwargs)
                    textMsg.additional_kwargs = extraKwargs;
                ...
                const msg = { role: messageRole, content: message.content };
                if (extraKwargs)
                    msg.additional_kwargs = extraKwargs;
                chatHistory.push(msg);
            }
        }
        else if (message.additional_kwargs) {
            const extraKwargs = collectKwargs(message.additional_kwargs);
            const msg = { role: messageRole, content: message.content };
            if (extraKwargs)
                msg.additional_kwargs = extraKwargs;
            chatHistory.push(msg);
            ...
            chatHistory.push({ role: messageRole, content: message.content });
        }
    }
...
const getArtifactTypeFromFilename = (filename) => {
    const extension = filename.toLowerCase().split('.').pop();
    return ARTIFACT_TYPES[extension || ''] || 'text';
};
exports.getArtifactTypeFromFilename = getArtifactTypeFromFilename;
// ─── Saving generated images to storage ──────────────────────────────────────
/** Saves base64 image data to storage and returns file information */
const saveBase64Image = async (outputItem, options) => {
    try {
...
const saveGeminiInlineImage = async (inlineItem, options) => {
    try {
        if (!inlineItem.data || !inlineItem.mimeType)
            return null;
        // Derive file extension from MIME type
        const mime = inlineItem.mimeType;
        const extension = mime.includes('jpeg') || mime.includes('jpg') ? 'jpg' : mime.includes('gif') ? 'gif' : mime.includes('webp') ? 'webp' : 'png';
...
exports.saveGeminiInlineImage = saveGeminiInlineImage;
// ─── Downloading container files from OpenAI ─────────────────────────────────
/** Downloads a file from an OpenAI container (used for file citations in responses). */
const downloadContainerFile = async (containerId, fileId, filename, modelNodeData, options) => {
    try {
        const credentialData = await (0, utils_1.getCredentialData)(modelNodeData.credential ?? '', options);
        const openAIApiKey = (0, utils_1.getCredentialParam)('openAIApiKey', credentialData, modelNodeData);
        if (!openAIApiKey) {
        ...
        const mimeType = (0, exports.getMimeTypeFromFilename)(filename);
...
exports.downloadContainerFile = downloadContainerFile;
 ...
 */
const replaceInlineDataWithFileReferences = (response, savedInlineImages) => {
...
exports.replaceInlineDataWithFileReferences = replaceInlineDataWithFileReferences;
 ...
 */
const extractArtifactsFromResponse = async (responseMetadata, modelNodeData, options) => {
    const artifacts = [];
    ...
    if (responseMetadata?.inlineData && Array.isArray(responseMetadata.inlineData)) {
        for (const inlineItem of responseMetadata.inlineData) {
            if (inlineItem.type === 'gemini_inline_data' && inlineItem.data && inlineItem.mimeType) {
                try {
                    const savedImageResult = await (0, exports.saveGeminiInlineImage)(inlineItem, options);
                    if (savedImageResult) {
                        artifacts.push({
                            type: (0, exports.getArtifactTypeFromFilename)(savedImageResult.fileName),
    ...
    for (const outputItem of responseMetadata.output) {
                ...
                if (contentItem.annotations && Array.isArray(contentItem.annotations)) {
                    for (const annotation of contentItem.annotations) {
                        if (annotation.type === 'container_file_citation' && annotation.file_id && annotation.filename) {
                            try {
                                const downloadResult = await (0, exports.downloadContainerFile)(annotation.container_id, annotation.file_id, annotation.filename, modelNodeData, options);
                                if (downloadResult) {
                                    const fileType = (0, exports.getArtifactTypeFromFilename)(annotation.filename);
        ...
        if (outputItem.type === 'image_generation_call' && outputItem.result) {
            try {
                const savedImageResult = await (0, exports.saveBase64Image)(outputItem, options);
                if (savedImageResult) {
...
exports.extractArtifactsFromResponse = extractArtifactsFromResponse;
 ...
 */
const addImageArtifactsToMessages = async (messages, options) => {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        for (const artifact of message.additional_kwargs.artifacts) {
            if (artifact.type && artifact.data && IMAGE_EXTENSIONS.includes(artifact.type.toLowerCase())) {
                imageArtifacts.push({
                    name: (0, validator_1.sanitizeFileName)(artifact.data),
        ...
        const alreadyPresent = nextMessage &&
            nextMessage.role === 'user' &&
            Array.isArray(nextMessage.content) &&
            nextMessage.content.some((item) => (item.type === 'stored-file' || item.type === 'image_url') &&
        ...
        for (const artifact of imageArtifacts) {
            try {
                const fileName = (0, validator_1.sanitizeFileName)(artifact.name);
        ...
        if (base64Contents.length > 0) {
            const base64Message = { role: 'user', content: base64Contents, _isTemporaryImageMessage: true };
            if (fileRefs.length > 0) {
...
exports.addImageArtifactsToMessages = addImageArtifactsToMessages;
...
const _addImagesToMessages = async (options, allowImageUploads) => {
    const imageContent = [];
    if (!allowImageUploads || !options?.uploads?.length)
        return imageContent;
    const imageUploads = (0, multiModalUtils_1.getImageUploads)(options.uploads);
