...
exports.updateFlowState =
    exports.addImageArtifactsToMessages =
    exports.extractArtifactsFromResponse =
    exports.replaceInlineDataWithFileReferences =
    exports.downloadContainerFile =
    exports.saveGeminiInlineImage =
    exports.saveBase64Image =
    exports.getArtifactTypeFromFilename =
    exports.getMimeTypeFromFilename =
    exports.getPastChatHistoryImageMessages =
    exports.getUniqueImageMessages =
    exports.normalizeMessagesForStorage =
    exports.revertBase64ImagesToFileRefs =
    exports.processMessagesWithImages =
        void 0
...
const saveImageToStorage = async (base64Data, mimeType, fileName, options) => {
    ...
    const { path, totalSize } = await (0, storageUtils_1.addSingleFileToStorage)(
        mimeType,
        imageBuffer,
        fileName,
        options.orgId,
        ...
        const alreadyInContent = messages.some((msg) => {
            ...
            if (Array.isArray(chatMsg.content)) {
                return chatMsg.content.some(
                    (item) => item.type === 'image_url' && image.type === 'image_url' && JSON.stringify(item) === JSON.stringify(image)
                )
            }
            ...
            try {
                ...
                for (const upload of uploads) {
                    if (upload.type === 'stored-file' && upload.mime.startsWith('image/')) {
                        ...
                        const fileLoaderNodeModule = await Promise.resolve().then(() =>
                            __importStar(require('../../nodes/documentloaders/File/File'))
                        )
                        // @ts-ignore
                        ...
                        messageWithFileUploads += `<doc name='${safeFileName}'>${(0, utils_1.handleEscapeCharacters)(
                            documents,
                            true
                        )}</doc>\n\n`
                    }
                ...
                if (contentItem.annotations && Array.isArray(contentItem.annotations)) {
                    for (const annotation of contentItem.annotations) {
                        if (annotation.type === 'container_file_citation' && annotation.file_id && annotation.filename) {
                            try {
                                const downloadResult = await (0, exports.downloadContainerFile)(
