...
class Agent_Agentflow {
    ...
    async run(nodeData, input, options) {
        ...
        try {
            ...
            if (humanInput) {
                ...
                if (isStreamable) {
                    response = await this.handleStreamingResponse(sseStreamer, llmNodeInstance, messages, chatId, abortController, isStructuredOutput, isLastNode);
                }
                else {
            ...
            if (response.response_metadata) {
                const { artifacts: extractedArtifacts, fileAnnotations: extractedFileAnnotations, savedInlineImages } = await (0, utils_2.extractArtifactsFromResponse)(response.response_metadata, newNodeData, options);
            ...
            const reasonContentObj = reasonContent !== undefined && reasonContent !== '' ? { thinking: reasonContent, thinkingDuration } : undefined;
            const costMetadata = await this.calculateUsageCost(model, modelConfig?.modelName, response.usage_metadata, additionalTokens);
            const output = this.prepareOutputObject(response, availableTools, finalResponse, startTime, endTime, timeDelta, usedTools, sourceDocuments, artifacts, additionalTokens, isWaitingForHumanInput, fileAnnotations, isStructuredOutput, reasonContentObj, costMetadata);
            ...
            const outputForStateProcessing = isStructuredOutput && typeof response === 'object' ? JSON.stringify(response, null, 2) : finalResponse;
            ...
            if (!runtimeChatHistory.length) {
                ...
                const imageInputMessages = messagesWithFileReferences.filter((msg) => msg.role === 'user' &&
    ...
    async extractBuiltInUsedTools(response, builtInUsedTools = []) {
        ...
        if (urlContextMetadata && urlContextMetadata.urlMetadata && Array.isArray(urlContextMetadata.urlMetadata)) {
            // Check for duplicates
            const isDuplicate = builtInUsedTools.find((tool) => tool.tool === 'urlContext' &&
        ...
        if (response.content && Array.isArray(response.content)) {
            for (let i = 0; i < response.content.length; i++) {
                ...
                if (item.type === 'executableCode' && item.executableCode) {
                    ...
                    let toolOutput = '';
                    // Check for duplicates
                    const isDuplicate = builtInUsedTools.find((tool) => tool.tool === 'codeExecution' &&
                        tool.toolInput?.language === language &&
                        tool.toolInput?.code === code);
                    if (isDuplicate) {
                        continue;
                    }
     ...
     */
    async handleMemory({ messages, memoryType, pastChatHistory, runtimeChatHistory, llmWithoutToolsBind, nodeData, userMessage, input, abortController, options, modelConfig }) {
     ...
     */
    async handleStreamingResponse(sseStreamer, llmNodeInstance, messages, chatId, abortController, isStructuredOutput = false, isLastNode = false) {
        let response = new messages_1.AIMessageChunk('');
        let reasonContent = '';
        let thinkingDuration;
        ...
        if (Array.isArray(response.content) && response.content.length > 0) {
            const hasNonTextContent = response.content.some((item) => item.type === 'inlineData' || item.type === 'executableCode' || item.type === 'codeExecutionResult');
     ...
     */
    prepareOutputObject(response, availableTools, finalResponse, startTime, endTime, timeDelta, usedTools, sourceDocuments, artifacts, additionalTokens = 0, isWaitingForHumanInput = false, fileAnnotations = [], isStructuredOutput = false, reasonContent, costMetadata) {
        const output = {
            content: finalResponse,
            timeMetadata: {
                start: startTime,
    ...
    async handleToolCalls({ response, messages, toolsInstance, sseStreamer, chatId, input, options, abortController, llmNodeInstance, isStreamable, isLastNode, iterationContext, isStructuredOutput = false, accumulatedReasonContent: initialAccumulatedReasonContent, accumulatedReasoningDuration: initialAccumulatedReasoningDuration }) {
        ...
        let newResponse;
        if (isStreamable) {
            newResponse = await this.handleStreamingResponse(sseStreamer, llmNodeInstance, messages, chatId, abortController, isStructuredOutput, isLastNode);
        }
        else {
        ...
        if (newResponse.tool_calls && newResponse.tool_calls.length > 0) {
            const { response: recursiveResponse, usedTools: recursiveUsedTools, sourceDocuments: recursiveSourceDocuments, artifacts: recursiveArtifacts, totalTokens: recursiveTokens, isWaitingForHumanInput: recursiveIsWaitingForHumanInput, accumulatedReasonContent: recursiveAccumulatedReasonContent, accumulatedReasoningDuration: recursiveAccumulatedReasoningDuration } = await this.handleToolCalls({
     ...
     * Handles tool calls and their responses, with support for recursive tool calling
     */
    async handleResumedToolCalls({ humanInput, humanInputAction, messages, toolsInstance, sseStreamer, chatId, input, options, abortController, llmWithoutToolsBind, isStreamable, isLastNode, iterationContext, isStructuredOutput = false }) {
        ...
        if (!response.tool_calls || response.tool_calls.length === 0) {
            ...
            const dur = typeof response.additional_kwargs?.reasoning_duration === 'number'
        ...
        let accumulatedReasoningDuration = (typeof response.additional_kwargs?.reasoning_duration === 'number' ? response.additional_kwargs.reasoning_duration : 0) +
