...
class SQLiteRecordManager_RecordManager {
    ...
    async init(nodeData, _, options) {
        ...
        const database = (0, validator_1.validateSQLitePath)(path_1.default.join(process.env.DATABASE_PATH ?? path_1.default.join((0, utils_1.getUserHome)(), '.flowise'), 'database.sqlite'));
...
CREATE INDEX IF NOT EXISTS group_id_index ON "${tableName}" (group_id);`);
            ...
            const checkColumn = await queryRunner.manager.query(`SELECT COUNT(*) as count FROM pragma_table_info('${tableName}') WHERE name='doc_id';`);
