...
function createDynamoDBClient(credentials, region) {
    ...
    return new client_dynamodb_1.DynamoDBClient(config);
}
// Helper function to build full key with optional prefix
function buildFullKey(key, keyPrefix) {
 ...
 */
class DynamoDBStoreTool extends tools_1.StructuredTool {
    constructor(dynamoClient, tableName, keyPrefix = '') {
        super();
        this.name = 'dynamodb_kv_store';
        this.description = 'Store a text value with a key in DynamoDB. Input must be an object with "key" and "value" properties.';
        this.schema = v3_1.z.object({
            key: v3_1.z.string().min(1).describe('The key to store the value under'),
            value: v3_1.z.string().describe('The text value to store')
        });
        this.dynamoClient = dynamoClient;
        this.tableName = tableName;
        this.keyPrefix = keyPrefix;
    }
    async _call({ key, value }) {
        try {
            validateKey(key);
 ...
 */
class DynamoDBRetrieveTool extends tools_1.StructuredTool {
    constructor(dynamoClient, tableName, keyPrefix = '') {
        super();
        this.name = 'dynamodb_kv_retrieve';
        this.description = 'Retrieve a value by key from DynamoDB. Returns JSON with value and timestamp. Specify which version to get (1=latest, 2=2nd latest, etc).';
        this.schema = v3_1.z.object({
            key: v3_1.z.string().min(1).describe('The key to retrieve the value for'),
            nthLatest: v3_1.z
                .string()
                .regex(/^\d+$/, 'Must be a positive number')
                .describe('Which version to retrieve: "1" for latest, "2" for 2nd latest, "3" for 3rd latest, etc. Use "1" to get the most recent value.')
                .optional()
                .default('1')
        });
        this.dynamoClient = dynamoClient;
 ...
 */
class AWSDynamoDBKVStorage_Tools {
    constructor() {
        this.loadMethods = {
            listTables: async (_nodeData, _options) => {
                try {
                    const credentials = await (0, awsToolsUtils_1.getAWSCredentials)(_nodeData, _options ?? {});
                    const region = _nodeData.inputs?.region || awsToolsUtils_1.DEFAULT_AWS_REGION;
                    ...
                    const tableChecks = await Promise.allSettled(listResponse.TableNames.map(async (tableName) => {
                        ...
                        return [
                            {
                                ...
                                name: ERROR_PLACEHOLDER,
                                description: `No tables with partition key "pk" and sort key "sk" found. ${invalidTables.length > 0 ? `Found ${invalidTables.length} table(s) with different schema.` : ''} Please create a table with these keys.`
                            }
                        ];
                    }
        ...
        this.baseClasses = [this.type, ...(0, utils_1.getBaseClasses)(DynamoDBStoreTool)];
        this.credential = {
    ...
    async init(nodeData, _, options) {
        ...
        const keyPrefix = nodeData.inputs?.keyPrefix || '';
        const operation = nodeData.inputs?.operation || Operation.STORE;
...
//# sourceMappingURL=AWSDynamoDBKVStorage.js.map