...
class AddContactTool extends BaseSendGridTool {
    ...
    async _call(arg) {
        ...
        try {
            ...
            if (params.firstName)
                contactData.first_name = params.firstName;
            if (params.lastName)
                contactData.last_name = params.lastName;
...
const createSendGridTools = (args) => {
    ...
    if (actions.includes('list_templates')) {
        tools.push(new ListTemplatesTool({
            apiKey,
            maxOutputLength,
            defaultParams
        }));
