...
const UpdateTaskSchema = v3_1.z.object({
    ...
    etag: v3_1.z.string().describe("The task's current @odata.etag value, from a prior get_task call. Required for optimistic concurrency."),
...
class UpdateTaskTool extends BasePlannerTool {
    constructor(args) {
        const toolInput = {
            name: 'update_task',
            description: "Update a Microsoft Planner task (e.g. its percent complete). Requires the task's current @odata.etag value, obtained from a prior get_task call, for optimistic concurrency.",
...
const createPlannerTools = (args) => {
    ...
    if (actions.includes('list_plans_for_group')) {
        tools.push(new ListPlansForGroupTool({
            accessToken,
            maxOutputLength,
            defaultParams
        }));
