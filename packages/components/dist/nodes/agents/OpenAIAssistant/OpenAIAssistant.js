...
class OpenAIAssistant_Agents {
    ...
    async run(nodeData, input, options) {
        ...
        const checkStorage = options.checkStorage ? options.checkStorage : undefined
        const updateStorageUsage = options.updateStorageUsage ? options.updateStorageUsage : undefined
...
const formatToOpenAIAssistantTool = (tool) => {
    const parameters = (0, utils_2.toolSchemaToJsonSchema)(tool.schema)
    // For strict tools, we need to:
    // 1. Set additionalProperties to false
    // 2. Make all parameters required
    // 3. Set the strict flag
    if (tool instanceof core_1.DynamicStructuredTool && tool.isStrict()) {
