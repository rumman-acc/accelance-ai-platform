...
class AWSChatBedrock_ChatModels {
    constructor() {
        ...
        this.inputs = [
            ...
            },
            {
                ...
                name: 'customModel',
                description: 'For imported, fine-tuned, or provisioned-throughput models. ' +
            ...
            },
            {
                ...
                name: 'endpointHost',
                type: 'string',
                description: 'Hostname-only override for a custom VPC endpoint or proxy ' +
            ...
            },
            {
                ...
                type: 'boolean',
                description: 'Force the global cross-region inference profile instead of the region-specific one. ' +
    ...
    async init(nodeData, _, options) {
        ...
        if (effectiveModel && !effectiveModel.startsWith('arn:aws:bedrock:')) {
            throw new Error(`"${effectiveModel}" is not a valid ARN. ` +
        ...
        if (effectiveModel?.includes(':imported-model/')) {
            const modelInfo = await (0, FlowiseAWSChatBedrockImported_1.getImportedModelInfo)(effectiveModel, iRegion, credentialConfig.credentials);
        ...
        const { modelId, applicationInferenceProfile } = await (0, utils_2.resolveBedrockModel)(effectiveCustomModel, iModel, iRegion, availableProfiles, useGlobalEndpoint);
        const obj = {
            region: iRegion,
            model: modelId,
            maxTokens: !isNaN(parseInt(iMax_tokens_to_sample, 10)) ? parseInt(iMax_tokens_to_sample, 10) : 200,
