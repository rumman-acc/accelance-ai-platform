...
const RouterContentType = v3_1.z.object({
    ...
    scenarios: v3_1.z.array(v3_1.z.string()).describe('One scenario description per outgoing branch, in the exact order the branches were given')
});
...
const validateAndRepairFlow = (nodes, edges, config) => {
    ...
    for (const node of nodes) {
        ...
        if (proposingTools.length > 0 && boundTool && !proposingTools.includes(boundTool)) {
            warnings.push(`"${node.data.label || node.id}" executes tool "${boundTool}", which doesn't match the tool(s) its proposing agent selected (${proposingTools.join(', ')}) -- verify this is intentional.`);
        }
    }
    ...
    for (const node of nodes) {
        ...
        if (!hasRealScenario || !hasInstructions) {
            warnings.push(`"${node.data.label || node.id}" is a router with no scenarios/instructions generated -- it will fail as soon as it runs. Open the node and fill these in manually.`);
        }
    }
    ...
    for (const node of nodes) {
        ...
        for (const t of agentTools) {
            ...
            if (hasCredentialField && !hasCredentialSet)
                toolNeedsCredentialButHasNone.add(t.agentSelectedTool);
        }
        const boundTool = node.data.inputs?.toolAgentflowSelectedTool;
        if (boundTool) {
            ...
            if (hasCredentialField && !hasCredentialSet)
                toolNeedsCredentialButHasNone.add(boundTool);
        }
    }
    const hasWriteCapableTool = Array.from(boundToolNames).some((name) => (0, toolActionRisk_1.isWriteCapableToolNode)(componentNodes[name]));
    const hasHumanInputNode = nodes.some((n) => n.data.name === 'humanInputAgentflow');
    if (hasWriteCapableTool && !hasHumanInputNode) {
        warnings.push('This flow uses at least one tool capable of mutating actions (sending, creating, deleting, etc.) but has no human-in-the-loop approval node anywhere -- review whether autonomous write access is intended before deploying.');
    }
...
const findProposingAgentTools = (node, nodes, edges) => {
    ...
    if (!viaApprovedGate && !directFromAgent)
        return []; // not this shape -- keep today's dedup default
...
`;
            ...
            const instructionsSysPrompt = `You are a workflow orchestrator building an agent step ("${node.data.label || node.id}") that is one part of a larger workflow for the user's overall request below.
...
`;
            ...
            for (const edge of edges) {
                ...
                if (!branchTargetLabels[idx])
                    branchTargetLabels[idx] = [];
