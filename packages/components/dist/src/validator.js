...
exports.sanitizeFileName = exports.validateSQLitePath = exports.validateVectorStorePath = exports.filterAllowedUploadMimeTypes = exports.validateMimeTypeAndExtensionMatch = exports.isUnsafeFilePath = exports.isPathTraversal = exports.isValidURL = exports.isValidUUID = void 0;
...
const validateFilename = (filename) => {
    ...
    if ((0, exports.isUnsafeFilePath)(filename)) {
...
const extractFileExtension = (filename) => {
    ...
    const extensionNormalizationMap = {
 ...
 * @returns {void} Throws an error if validation fails
 */
const validateMimeTypeAndExtensionMatch = (filename, mimetype) => {
    ...
    const expectedExt = (0, utils_1.mapMimeTypeToExt)(mimetype);
    if (!expectedExt) {
...
exports.validateMimeTypeAndExtensionMatch = validateMimeTypeAndExtensionMatch;
/**
 ...
 */
const filterAllowedUploadMimeTypes = (mimeTypes) => {
    if (!Array.isArray(mimeTypes))
        return [];
    return mimeTypes.map((m) => (typeof m === 'string' ? m.trim() : '')).filter((m) => m !== '' && (0, utils_1.isAllowedUploadMimeType)(m));
};
exports.filterAllowedUploadMimeTypes = filterAllowedUploadMimeTypes;
/**
 * Get allowed base directories for vector store operations
 * @returns {string[]} Array of allowed base directory paths
 */
const getAllowedVectorStoreBaseDirs = () => {
    const allowedDirs = [];
    // Allow user home .flowise directory
    const userHome = (0, utils_1.getUserHome)();
    allowedDirs.push(path_1.default.join(userHome, '.flowise'));
    // Allow configured blob storage path if set
    if (process.env.BLOB_STORAGE_PATH) {
        allowedDirs.push(path_1.default.resolve(process.env.BLOB_STORAGE_PATH));
    }
    return allowedDirs;
};
/**
 ...
 */
const validateVectorStorePath = (userProvidedPath) => {
    if (process.env.PATH_TRAVERSAL_SAFETY === 'false') {
        if (!userProvidedPath || userProvidedPath.trim() === '') {
            return path_1.default.join((0, utils_1.getUserHome)(), '.flowise', 'vectorstore');
        }
        const bypassPath = userProvidedPath.trim();
        return path_1.default.isAbsolute(bypassPath) ? bypassPath : path_1.default.resolve(path_1.default.join((0, utils_1.getUserHome)(), '.flowise', bypassPath));
    }
    ...
    if (path_1.default.isAbsolute(basePath)) {
        resolvedPath = path_1.default.resolve(basePath);
    }
    else {
        // Relative paths are resolved within the .flowise directory for safety
        resolvedPath = path_1.default.resolve(path_1.default.join((0, utils_1.getUserHome)(), '.flowise', basePath));
    }
    ...
    const isWithinAllowedDir = allowedDirs.some((allowedDir) => {
        ...
        return normalizedResolved === normalizedAllowed || normalizedResolved.startsWith(normalizedAllowed + path_1.default.sep);
    });
    if (!isWithinAllowedDir) {
        throw new Error(`Invalid path: path must be within allowed directories (${allowedDirs.join(', ')}). ` + `Attempted path: ${resolvedPath}`);
    }
    return resolvedPath;
};
exports.validateVectorStorePath = validateVectorStorePath;
const getAllowedSQLiteBaseDirs = () => {
    const dirs = [path_1.default.join((0, utils_1.getUserHome)(), '.flowise')];
    if (process.env.DATABASE_PATH) {
        dirs.push(path_1.default.resolve(process.env.DATABASE_PATH));
    }
    return dirs;
};
const normalizePlatformPath = (p) => {
    const n = path_1.default.normalize(p);
    return process.platform === 'win32' ? n.toLowerCase() : n;
};
const isPathWithinAllowedSQLiteDirs = (resolvedPath, allowedDirs) => {
    const normalizedResolved = normalizePlatformPath(resolvedPath);
 ...
 */
const validateSQLitePath = (userProvidedPath) => {
    const allowedDirs = getAllowedSQLiteBaseDirs();
    const defaultDir = allowedDirs[0];
    if (process.env.PATH_TRAVERSAL_SAFETY === 'false') {
        if (!userProvidedPath || userProvidedPath.trim() === '') {
            return path_1.default.join(defaultDir, 'database.sqlite');
        }
        const bypassPath = userProvidedPath.trim();
        return path_1.default.isAbsolute(bypassPath) ? bypassPath : path_1.default.resolve(path_1.default.join(defaultDir, bypassPath));
    }
    if (!userProvidedPath || userProvidedPath.trim() === '') {
    ...
    const basePath = userProvidedPath.trim();
    if (basePath.includes('..'))
        throw new Error('Invalid SQLite path: path traversal attempt detected');
    if (basePath.toLowerCase().includes('%2e') || basePath.toLowerCase().includes('%2f') || basePath.toLowerCase().includes('%5c'))
    ...
    if (/^[a-zA-Z]:\\/.test(basePath))
        throw new Error('Invalid SQLite path: Windows absolute paths are not allowed');
    if (/^\\\\[^\\]/.test(basePath))
        throw new Error('Invalid SQLite path: UNC paths are not allowed');
    if (/^\\\\\?\\/.test(basePath))
        throw new Error('Invalid SQLite path: extended-length paths are not allowed');
    const resolvedPath = path_1.default.isAbsolute(basePath) ? path_1.default.resolve(basePath) : path_1.default.resolve(path_1.default.join(defaultDir, basePath));
    if (resolvedPath.includes('..'))
        throw new Error('Invalid SQLite path: path traversal detected in resolved path');
    if (!isPathWithinAllowedSQLiteDirs(resolvedPath, allowedDirs)) {
        throw new Error(`Invalid SQLite path: path must be within allowed directories (${allowedDirs.join(', ')}). Attempted path: ${resolvedPath}`);
    }
    return resolvedPath;
};
exports.validateSQLitePath = validateSQLitePath;
/**
