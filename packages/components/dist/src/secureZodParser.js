...
class SecureZodSchemaParser {
    ...
    static validateTypeInfo(typeInfo) {
    ...
    static extractArrayWithModifiers(typeStr) {
    ...
    static extractObjectWithModifiers(typeStr) {
    ...
    static parseComplexArguments(argsStr) {
        // Remove outer parentheses
        const inner = argsStr.slice(1, -1).trim();
        if (!inner)
            return [];
    ...
    static buildZodType(typeInfo) {
        ...
        if (typeInfo.isNestedObject) {
            let zodType = this.buildZodSchema(typeInfo.objectSchema);
        ...
        if (typeInfo.isArrayOfObjects) {
            ...
            let zodType = v3_1.z.array(objectSchema);
            // Apply modifiers if present
            if (typeInfo.modifiers) {
                zodType = this.applyModifiers(zodType, typeInfo.modifiers);
            }
        ...
        if (typeInfo.isSimpleArray) {
            ...
            let zodType = v3_1.z.array(innerZodType);
            // Apply modifiers if present
            if (typeInfo.modifiers) {
                zodType = this.applyModifiers(zodType, typeInfo.modifiers);
            }
        ...
        switch (typeInfo.base) {
            ...
            case 'enum':
                if (typeInfo.baseArgs && typeInfo.baseArgs[0] && Array.isArray(typeInfo.baseArgs[0])) {
                    ...
                    zodType = v3_1.z.enum(enumValues);
                }
                else {
    ...
    static applyModifiers(zodType, modifiers) {
