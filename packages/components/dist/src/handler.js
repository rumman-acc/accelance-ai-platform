...
const additionalCallbacks = async (nodeData, options) => {
    try {
        if (!options.analytic && !(0, tracingEnv_1.tracingEnvEnabled)())
            return [];
        ...
        for (const provider in analytic) {
            ...
            if (providerStatus) {
                const credentialData = envCredentials[provider] ?? (await (0, utils_1.getCredentialData)(analytic[provider].credentialId ?? '', options));
...
class AnalyticHandler {
    ...
    async init() {
        ...
        try {
            ...
            if (!hasAnalyticsConfig && !(0, tracingEnv_1.tracingEnvEnabled)())
                return;
            ...
            for (const provider in analytic) {
                const providerStatus = analytic[provider].status;
                if (providerStatus) {
                    const credentialData = envCredentials[provider] ??
