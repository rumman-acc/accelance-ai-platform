...
exports.JsonOutputToolsParser = exports.ToolCallingAgentOutputParser = exports.XMLAgentOutputParser = exports.createReactAgent = exports.formatAgentSteps = exports.AgentExecutor = exports.AgentExecutorIterator = exports.formatToolError = exports.TOOL_ARGS_PREFIX = exports.ARTIFACTS_PREFIX = exports.SOURCE_DOCUMENTS_PREFIX = void 0;
...
const log_1 = require("@langchain/classic/agents/format_scratchpad/log");
 ...
 */
const formatToolError = (errorMessage, params) => {
    return errorMessage + exports.TOOL_ARGS_PREFIX + JSON.stringify(params);
};
            ...
            const newSteps = await Promise.all(actions.map(async (action) => {
                ...
                try {
                    ...
                    if (tool) {
                        ...
                        let toolInput;
                        if (typeof toolOutput === 'string' && toolOutput.includes(exports.TOOL_ARGS_PREFIX)) {
                            const splitArray = toolOutput.split(exports.TOOL_ARGS_PREFIX);
                        ...
                        usedTools.push({
                            tool: tool.name,
                            toolInput: toolInput ?? action.toolInput,
                            toolOutput
                        });
                    }
                    else {
                ...
                if (typeof observation === 'string' && observation.includes(exports.TOOL_ARGS_PREFIX)) {
                    const observationArray = observation.split(exports.TOOL_ARGS_PREFIX);
                    observation = observationArray[0];
                }
                return { action, observation: observation ?? '' };
            }));
...
const formatAgentSteps = (steps) => steps.flatMap(({ action, observation }) => {
    const create_function_message = (observation, action) => {
        ...
        return new messages_1.FunctionMessage({ content, name: action.tool });
    };
...
class ToolCallingAgentOutputParser extends AgentMultiActionOutputParser {
    ...
    async parseResult(generations) {
        if ('message' in generations[0] && (0, messages_1.isAIMessage)(generations[0].message)) {
