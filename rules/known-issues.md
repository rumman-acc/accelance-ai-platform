...
## #011 — `PUT /api/v1/organization` has no permission check and accepts any field via a blind merge

**Symptom:** Not user-facing yet (no UI calls this route with attacker-controlled input today), but discovered while adding the Org → Workspace → Agent analytics cascade (2026-08-13): `organization.route.ts`'s `router.put('/', organizationController.update)` has no `checkPermission(...)` middleware at all, unlike every other mutating enterprise route (`workspace.route.ts`, `role.route.ts`, etc.). `OrganizationService.updateOrganization` does `queryRunner.manager.merge(Organization, oldOrganizationData, newOrganizationData)` with no field whitelist, so any authenticated user (not just an org owner/admin) who can reach this route can overwrite arbitrary `Organization` columns — including the new `analytic` column — by PUTing `{ id, analytic: '{"...": "..."}' }`.

**Root cause:** the route predates the RBAC middleware convention used elsewhere and was never retrofitted.

**Not fixed:** left as-is rather than bolted on here, since tightening it risks breaking whatever currently relies on the unrestricted PUT (org name/slug edits, Stripe customer/subscription id updates funnel through the same service method). The new analytics-cascade work added a properly-gated sibling route instead (`PATCH /organization/analytic`, behind `organization:analytics-manage` + `feat:organization-analytics`) so at least the analytics path is safe — but the old generic PUT still lets any authenticated user bypass that gate entirely by hitting `PUT /` directly with `{ analytic: ... }` in the body.

**Prevention:** any new mutating enterprise route should be checked against `workspace.route.ts` as the reference pattern (feature-flag check + `checkPermission(...)` on every non-GET route) before assuming an existing sibling route already does the same. Fixing this properly means: add `checkPermission('organization:update')` (new permission) to the PUT route, and switch `updateOrganization` to whitelist the fields a generic update is allowed to touch (name/slug only), moving every other field (analytic, customerId, subscriptionId, sso_config, ...) to its own scoped endpoint the way `updateOrganizationAnalytic` now does.
