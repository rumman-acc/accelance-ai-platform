...
| Native connector library — batch 2 (proposed, not yet scoped/approved) | 🔴 To be built | Batch 1 (this session, 2026-08-11) added Salesforce, HubSpot, Discord, Twilio, Airtable, DocuSign, Shopify (native connectors) + Notion, Linear, Sentry, Figma, Browserbase (native MCP servers) — see the market-gap row above. A batch 2 shortlist by category, pending prioritization: **Support/ticketing** (Zendesk, Intercom, Freshdesk); **Project/PM** (Asana, Trello, Monday.com, ClickUp); **Marketing/email** (Mailchimp, SendGrid, Klaviyo); **Comms** (Zoom, Google Meet, Telegram, WhatsApp Business); **Dev/infra** (GitLab, Bitbucket, CircleCI, Vercel, Datadog, PagerDuty); **Finance/accounting** (QuickBooks, Xero); **Cloud storage** (Dropbox, Box, OneDrive); **Data/analytics** (Snowflake, BigQuery, Segment, Amplitude, Mixpanel). Needs the same scoping pass as batch 1 (pick auth pattern per service, real endpoints, credential shape) before building — not started | Matches the "cover the common SaaS surface natively" direction rather than funneling everything through aggregators |
