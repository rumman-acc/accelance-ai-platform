...
| Market-gap round 2 (2026-08-12): Azure, Microsoft 365/Graph beyond Outlook/Teams/OneDrive, Anthropic/OpenAI ecosystems, other enterprise "big fish" | 🔴 To be built — researched, not started | Must-have shortlist per market research (see conversation, 2026-08-12): Azure Blob Storage, Azure Key Vault, Azure DevOps, Entra ID (Azure AD), SharePoint, Excel Online, Planner, ServiceNow, Confluence, Okta, Jira Service Management — 11 connectors, same OAuth2/service-principal or REST+token patterns already used elsewhere in this table. Plus two connectors of a new kind: **Claude-as-tool** and **GPT-as-tool** (call the Anthropic Messages API / OpenAI Responses API from inside a flow as a nested sub-agent step) — the actual finding here is that neither Anthropic nor OpenAI publishes a broad enterprise-tool MCP catalog worth wrapping; this pair is the only real native-connector opportunity from that angle. Confirmed via research: Azure Cognitive Search and Snowflake are both better served by this platform's existing RAG/SQL-chain nodes (reinforces the earlier Snowflake/BigQuery exclusion, doesn't overturn it); SAP/Workday are real but high-cost/low-near-term-ROI (negotiated API access, complex OAuth2+SCIM) — deliberately deferred, not forgotten | Rounds out enterprise-tier coverage (identity, ITSM, Microsoft ecosystem) that batches 1–2 didn't reach |
