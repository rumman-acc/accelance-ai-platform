...
| Tool/node catalog performance at 1000+ scale (virtualization, lazy-loaded schemas, debounced search) | 🔴 To be built | `AddNodes.jsx` renders the full catalog via plain `.map()` in MUI Accordions, no virtualization lib in `packages/ui` (no react-window/react-virtual/Virtuoso), fuzzy search re-scores synchronously per keystroke with no debounce | Standard large-catalog UX pattern (virtualized list + paginated/lazy server-side catalog for aggregator-scale tools, e.g. Composio/Zapier-class integration counts) |
