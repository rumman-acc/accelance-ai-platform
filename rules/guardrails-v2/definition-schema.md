...
| `hooks` | text (`'pre'\|'post'\|'both'`) | yes | When `placement='attached'`. The 3 built-in keys (`egress_filtering`=pre, `prompt_injection_defense`=post, `confused_deputy_prevention`=pre) are wired via hardcoded call sites in their own node files, not by reading this column. **2026-08-21, Phase 3 update:** this column is now the FIRST real, data-driven consumer for custom definitions — `createCustomDefinition` requires it (single-select `'pre'` or `'post'` only; `'both'` explicitly deferred, see `phase3-authoring.md`), and `runAttachedGuardrails.ts`'s `runCustomToolCallGuardrails` reads it at runtime to decide whether a custom `regex_match` guardrail checks the tool's outgoing arguments (`pre`) or its result (`post`). Live-verified via direct invocation of the real compiled node/dispatcher against a real DB: a `pre`-hooked definition blocked before the wrapped tool's `_call` ever ran (based on args); a `post`-hooked definition ran after and redacted based on the result, even when the same pattern was absent from the args entirely. |
| `paramSchema` | text (JSON) | no | Descriptive schema for a future config form. Not rendered by anything in Phase 1. **2026-08-21 update:** `prompt_injection_defense`'s seeded v1 row (`{"pattern":"string","action":"string"}`) didn't match the shipped `PromptInjectionDefense.ts` node (zero configurable params beyond `observeMode` — the documented match-all approximation). Closed via the versioning mechanism this table describes below, not an in-place edit: a v2 row (`paramSchema:{}`, `defaultParams:{}`) now supersedes v1 — see `phase2-canvas.md`'s "Config panel round-trip" section and migrations `1798000000000..1798000000003-SupersedePromptInjectionDefenseParamSchema`. Serves as the first real, live example of this table's versioning model in use. |
