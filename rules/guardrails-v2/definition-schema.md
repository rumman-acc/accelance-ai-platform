...
| `allowedHosts` | text (JSON array) | yes | Node categories/baseClasses this may attach to, when `placement='attached'`. Unused by any Phase 1 code path (no real anchor exists yet) — present so Phase 2 doesn't need a migration to add it. |
| `hooks` | text (JSON: `'pre'\|'post'\|'both'`) | yes | When `placement='attached'`. Same Phase-1-inert status as `allowedHosts`. |
